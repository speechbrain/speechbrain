...
class MultiheadRelativeAttention(nn.MultiheadAttention):
    ...
    erdim: int
        dimension of the relative position matrix
    dropout : float
        a Dropout layer on attn_output_weights (default: 0.0).
    bias : bool
        add bias as module parameter (default: True).
    ...
    def __init__(
        ...
        self.Er = nn.Parameter(torch.randn(erdim, self.embed_dim))
...
def multihead_relativeattention_forward(
    ...
    if (relpos_mat is not None):
        er = _get_valid_embedding(relpos_mat, query.shape[1])
...
def _skew(srel):
    ...
    qe = qe*mask
    pad_srel = nn.functional.pad(qe, (1, 0))  #Pad to the leftmost side of the matrix

    qe = torch.reshape(qe, (qe.reshape[0], qe.reshape[2], qe.reshape[1])) # Reshape the matrix from (L, L+1) to (L+1, L)
    srel = qe[:, 1:, :]
...
def _scaled_dot_product_attention(q, k, v, er, attn_mask=None, dropout_p=0.0):
    ...
    print(q.size(), er.size())
    srel = torch.einsum('hld,md->hlm', q, er)
    srel = _skew(srel)
