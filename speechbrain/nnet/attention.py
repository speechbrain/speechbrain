...
class ContentBasedAttention(nn.Module):
    ...
    >>> out_tensor, out_weight = net(enc_tensor, enc_len, dec_tensor, init_params=True)
    >>> out_tensor.shape
    torch.Size([4, 5])
    """
...
class LocationAwareAttention(nn.Module):
    ...
    >>> enc_len = torch.ones([4]) * 10
    >>> dec_tensor = torch.rand([4, 25])
    >>> net = ContentBasedAttention(enc_dim=20, dec_dim=25, attn_dim=30, output_dim=5, conv_channels=10, kernel_size=3)
    >>> out_tensor, out_weight = net(enc_tensor, enc_len, dec_tensor, init_params=True)
    >>> out_tensor.shape
    torch.Size([4, 5])
    """
