...
def get_si_snr_stereo_with_pitwrapper(source, estimate_source):
    """
    This function wraps si_snr calculation with the speechbrain pit-wrapper
    ...
    pit_si_snr = PitWrapper(cal_si_snr_stereo)
    loss, perms = pit_si_snr(source, estimate_source)

    return loss.mean()


def cal_si_snr_stereo(source, estimate_source):
    """Calculate SI-SNR
    Arguments:
        source: [T, B, C],
            where B is batch size, T is the length of the sources, C is the number of sources
    ...
    source = source.reshape(2, -1, source.size(-2), source.size(-1))
    estimate_source = estimate_source.reshape(
        2, -1, estimate_source.size(-2), estimate_source.size(-1)
    )

    source_lengths = torch.tensor(
        [estimate_source.shape[1]] * estimate_source.shape[2]
    ).to(device)
    # mask = get_mask(source, source_lengths)
    # estimate_source *= mask

    num_samples = (
        source_lengths.contiguous().reshape(1, 1, -1, 1).float()
    )  # [1, B, 1]
    mean_target = torch.sum(source, dim=1, keepdim=True) / num_samples
    mean_estimate = (
        torch.sum(estimate_source, dim=1, keepdim=True) / num_samples
    )
    zero_mean_target = source - mean_target
    zero_mean_estimate = estimate_source - mean_estimate
    # mask padding position along T
    ...
    s_estimate = zero_mean_estimate  # [T, B, C]
    # s_target = <s', s>s / ||s||^2
    dot = torch.sum(s_estimate * s_target, dim=1, keepdim=True)  # [1, B, C]
    s_target_energy = (
        torch.sum(s_target ** 2, dim=1, keepdim=True) + EPS
    )  # [1, B, C]
    proj = dot * s_target / s_target_energy  # [T, B, C]
    # e_noise = s' - s_target
    e_noise = s_estimate - proj  # [T, B, C]
    # SI-SNR = 10 * log_10(||s_target||^2 / ||e_noise||^2)
    si_snr_beforelog = torch.sum(proj ** 2, dim=1) / (
