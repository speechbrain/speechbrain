    ...
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - initialize_with (type: str, optional, Default:None):
...
class linear_combination(nn.Module):
    """
     ...
     Example:   import torch
                ...
                # config dictionary definition
                config={'class_name':'lib.nnet.architectures.linear_combination'}

                # Initialization of the class
                linear_comb=linear_combination(config,first_input=[inp1,inp2,inp3])
    ...
    def __init__(
        ...
        for i in range(len(first_input)-1):
            self.w.append(nn.Linear(first_input[i].shape[1], dim_out, bias=self.bias))
    ...
    def forward(self, input_lst):
        ...
        for i in range(len(input_lst)-1):
            ...
            if x.shape[1] != wx.shape[1]:
                ...
                add_time_steps=(math.ceil(time_red_factor)*x.shape[1])-wx.shape[1]
                
                shape_add=list(wx.shape)
                shape_add[1]=add_time_steps
                
                wx = torch.cat([wx, torch.zeros(tuple(shape_add)).to(x.device)],dim=1)                
...
class conv(nn.Module):
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - stride (type: int_list(1,inf), optional: default: 1,1):
                           ...
                           - dilation (type: int_list(1,inf), optional: default: 1,1):
                           ...
                           - padding (type: int_list(1,inf), optional: default: None):
                           ...
                           - padding_mode (one_of(circular,zeros), optional: default: zeros):
                               This flag specifies the type of padding.
                               See torch.nn documentation for more information.
                               
                            - groups (type:int(1,inf), optional: default: zeros):
        ...
        if first_input is not None:
            ...
            if self.conv2d:
                if len(first_input[0].shape)==3:
                    ...
                    self.in_channels=first_input[0].shape[2]*first_input[0].shape[3]
    ...
    def forward(self, input_lst):
        ...
        if self.reshape_conv1d:
            ...
            if len(or_shape)==4:
                x = x.reshape(or_shape[0]*or_shape[2],or_shape[1],or_shape[-1])
        
            if len(or_shape)==5:
                x = x.reshape(or_shape[0]*or_shape[2]*or_shape[3],or_shape[1],or_shape[-1]) 
        ...
        if self.conv2d:
            ...
            if self.reshape_conv2d:
                ...
                if len(or_shape)==5:
                    x = x.reshape(or_shape[0],or_shape[1],or_shape[2]*or_shape[3],or_shape[-1]) 
        ...
        if self.padding is None:
            x = self.manage_padding(x,self.kernel_size, 
                                      self.dilation, 
                                      self.stride)
        ...
        if self.reshape_conv1d:
            ...
            if len(or_shape)==4:
                wx = wx.reshape(or_shape[0], wx.shape[1], or_shape[2], wx.shape[-1])
        
            if len(or_shape)==5:
                wx = wx.reshape(or_shape[0], wx.shape[1], or_shape[2], or_shape[3],wx.shape[-1])
    ...
    def compute_conv1d_shape(L_in, kernel_size, dilation, stride):
        ...
        Input (call):    - L_in (type: int, mandatory):
                         ...
                         - dilation (type: int, mandatory):
                              it is the dilation factor used for the convolution.
    ...
    def manage_padding(self,x,kernel_size,dilation,stride):
     ...
     Example:   import torch
        ...
        if stride[-1] > 1:
            n_steps=math.ceil(((L_in-kernel_size[-1]*dilation[-1])/stride[-1])+1)
            L_out = stride[-1]*(n_steps-1)+kernel_size[-1]*dilation[-1]
            padding= [kernel_size[-1] // 2, kernel_size[-1] // 2]
                   
        else:
            L_in = x.shape[-1]
            L_out = self.compute_conv1d_shape(L_in,kernel_size[-1],dilation[-1],stride[-1])
        ...
        if self.conv2d:
            ...
            if stride[-2] > 1:
                n_steps=math.ceil(((L_in-kernel_size[-2]*dilation[-2])/stride[-2])+1)
                L_out = stride[-2]*(n_steps-1)+kernel_size[-2]*dilation[-2]
                padding= padding + [kernel_size[-2] // 2, kernel_size[-2] // 2]
                
            else:    
                L_in = x.shape[-2]
                L_out = self.compute_conv1d_shape(L_in,kernel_size[-2],dilation[-2],stride[-2])
...
class SincConv(nn.Module):
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - sample_rate (type: int(1,inf), optional: default: 16000):
                               it is the sampling frequency of the input
                               waveform.
                               
                            - min_low_hz (type: float(0,inf), optional: default: 50):
                               it is the mininum frequnecy (in Hz) that a learned
                               filter can have.

                            - min_band_hz (type: float(0,inf), optional: default: 50):
    ...
    def __init__(
        ...
        if first_input is not None:
            ...
            if len(first_input[0].shape) > 2:
                ...
                err_msg = "SincConv only support one input channel (here, in_channels = {%i})" % (len(first_input[1:-1].shape))
    ...
    def forward(self, input_lst):
        ...
        self.filters = (band_pass).view(
            self.out_channels, 1, self.kernel_size).to(x.device)
        ...
        wx = F.conv1d(x, self.filters, stride=self.stride,
    ...
    def manage_padding(self,x,kernel_size,dilation,stride):
     ...
     Example:   import torch
                ...
                # Executing computations
                padded_tensor=sincnet.manage_padding(inp_tensor.unsqueeze(0),129,1,1)
                print(inp_tensor.shape)
                print(padded_tensor.shape)
        ...
        if stride > 1:
            ...
            L_in = x.shape[-1]
            L_out = self.compute_conv1d_shape(L_in,kernel_size,dilation,stride)
            padding = [(L_in - L_out) // 2, (L_in - L_out) // 2]
...
class RNN_basic(nn.Module):
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - rnn_type (type:rnn,lstm,gru,ligru,qrnn, mandatory):
    ...
    def __init__(
        ...
        if self.rnn_type == 'rnn':
            self.rnn = torch.nn.RNN(input_size=self.fea_dim,
        ...
        if self.rnn_type == 'lstm':
            self.rnn = torch.nn.LSTM(input_size=self.fea_dim,
                                    hidden_size=self.n_neurons,
                                    num_layers=self.num_layers,
                                    bias=self.bias,
                                    dropout=self.dropout,
        ...
        if self.rnn_type == 'gru':
            self.rnn = torch.nn.GRU(input_size=self.fea_dim,
                                    hidden_size=self.n_neurons,
                                    num_layers=self.num_layers,
                                    bias=self.bias,
                                    dropout=self.dropout,
        ...
        if self.rnn_type == 'ligru':
            self.rnn=liGRU(input_size=self.fea_dim,
                           hidden_size=self.n_neurons,
                           num_layers=self.num_layers,
                           dropout=self.dropout,
                           bidirectional=self.bidirectional
        ...
        if self.rnn_type == 'qrnn':
            ...
            try:
                ...
                err_msg = ("QRNN is not installed. Please run " 
                "pip install cupy pynvrtc git+https://github.com/salesforce/pytorch-qrnn ."
                "Go to https://github.com/salesforce/pytorch-qrnn for more info." 
                )
                logger_write(err_msg, logfile=logger)
            ...
            self.rnn = QRNN(input_size=self.fea_dim, 
                            hidden_size=self.n_neurons,
                            num_layers=self.num_layers, 
                            dropout=self.dropout)
...
class liGRU(nn.Module):
    """
     ...
     Input (call): - x (type, torch.Tensor, mandatory):
                       x is torch.tensor that we want to transform with the RNN. 
                       The tensor must be in one of the following format: 
                       [batch,channels,time]. Note that we can have up to
                       three channels. 
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
            ...
            self.wh.append(nn.Linear(current_dim, self.hidden_size, bias=False))
            self.wz.append(nn.Linear(current_dim, self.hidden_size, bias=False))

            # Recurrent connections
            self.uh.append(nn.Linear(self.hidden_size, self.hidden_size, bias=False))
            self.uz.append(nn.Linear(self.hidden_size, self.hidden_size, bias=False))
        ...
        for i in range(self.num_layers):
            ...
            if self.training:
                drop_mask = torch.bernoulli(
                    torch.Tensor(h_init.shape[0], h_init.shape[1]).fill_(1. - self.dropout)
                )
            else:
                drop_mask = torch.FloatTensor([1. - self.dropout])
            ...
            wh_bn = self.bn_wh[i](wh.view(wh.shape[0] * wh.shape[1], wh.shape[2]))
            wh = wh_bn.view(wh.shape[0], wh.shape[1], wh.shape[2])

            wz_bn = self.bn_wz[i](wz.view(wz.shape[0] * wz.shape[1], wz.shape[2]))
...
class activation(nn.Module):
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - negative_slope (type: float, optional, Default:0.01):
    ...
    def __init__(
        ...
        self.expected_options = {
            "class_name": ("str", "mandatory"),
            "act_type": ("one_of(relu,leaky_relu,relu_6,r_relu,p_relu,elu,"+
        ...
        if first_input is not None:
            ...
            if len(first_input[0].shape) > 5 or len(first_input[0].shape) < 1:
                ...
                err_msg = (
                    'The input of "activation must be a tensor with one of the  '
            ...
            if self.act_type == 'leaky_relu':
                self.act = torch.nn.LeakyReLU(negative_slope=self.negative_slope,inplace=self.inplace)
                
            if self.act_type == 'relu_6':
                self.act = torch.nn.ReLU6(inplace=self.inplace)
                
            if self.act_type == 'r_relu':
                self.act = torch.nn.RReLU(lower=self.lower, upper= self.upper,inplace=self.inplace)
            ...
            if self.act_type == 'softplus':
                self.act =  torch.nn.Softplus(beta=self.beta, threshold=self.threshold)            

            if self.act_type == 'soft_sign':
                self.act =  torch.nn.SoftSign()    
                
            if self.act_type == 'threshold':    
                self.act = torch.nn.Threshold(self.threshold, self.value, inplace=self.inplace)

            if self.act_type == 'hard_tanh':    
                self.act = torch.nn.Hardtanh(min_val=self.min_val, max_val=self.max_val, inplace=self.inplace)     
    ...
    def forward(self, input_lst):  
        ...
        if self.reshape:
               ...
               if len(dims) == 5:
                   x_act = x_act.reshape(dims[0],dims[1],dims[2],dims[3],dims[4])
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - act_type (type:relu,leaky_relu,relu_6,r_relu,
                            ...
                            - drop_rate (type: float(0,1), optional, Default:0.0):
                               it is the dropout factor.
