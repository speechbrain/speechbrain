...
def generate_wer_filename(
    wer_file: str, label: str, separator: str = "_"
) -> str:
    """Generates a new wer filename with the given label.

    The new wer filename is generated by adding the first part of
    wer_file (i.e. the part before the extension) and the label
    separated by a separator. The extension is kept the same.
    ...
    wer_file : str
        The wer file name.
    label : str
        The label to add to the wer file name.
    separator : str
        The separator to use between the wer file name and the given label.

    Returns
    -------
    new_wer_file : str
        The new wer file name.

    Example
    -------
    >>> hparams = {'wer_file': 'wer.txt'}
    >>> hparams['wer_file'] = generate_wer_filename(hparams['wer_file'], 'train')
    >>> hparams['wer_file']
    'wer_train.txt'
    """
    wer_file = wer_file.split(".")
    prefix_wer_file = "".join(wer_file[:-1])
    extension_wer_file = wer_file[-1]

    label_and_extension = f"{separator}{label}.{extension_wer_file}"
