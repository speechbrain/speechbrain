"""Transformer implementation for Mossformer2
...
from ..utils.normalization import CLayerNorm, LayerNorm, ScaleNorm
    ...
    def cal_attention(self, x, quad_q, lin_q, quad_k, lin_k, v, u, mask=None):
        ...
        if self.causal:
            causal_mask = torch.ones(
                (g, g), dtype=torch.bool, device=device
            ).triu(1)
...
class FLASHTransformer_DualA_FSMN(nn.Module):
    ...
    def _build_repeats(
        self, in_channels, out_channels, lorder, hidden_size, repeats=1
    ):
        repeats = [
