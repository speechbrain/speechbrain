...
class ScoreModel(nn.Module):
    ...
    Parameters:
    - backbone (str): Name of the backbone network architecture.
    - sde (str): Identifier of the SDE to use for diffusion sampling.
    - lr (float): Learning rate for optimizer.
    - ema_decay (float): Exponential moving average decay rate.
    - t_eps (float): Minimum time offset for numerical stability.
    - num_eval_files (int): Number of files to evaluate during validation.
    - loss_type (str): One of "score_matching", "denoiser", or "data_prediction".
    - loss_weighting (str): Weighting scheme for the loss (e.g., "sigma^2").
    - network_scaling (str or None): Scaling applied to network output.
    - c_in, c_out, c_skip (str): Coefficients for signal combinations.
    - sigma_data (float): Data noise standard deviation for EDM.
    - l1_weight (float): Weight for L1 term in data_prediction loss.
    - pesq_weight (float): Weight for PESQ loss term.
    - sr (int): Sample rate of audio.
    - num_frames (int): Number of time-frequency frames.
    ...
    def forward(self, x_t, y, t):
        ...
        Parameters:
        - x_t (torch.Tensor): The perturbed spectrogram at time `t`, of shape (B, 1, F, T).
        - y (torch.Tensor): The noisy input spectrogram of shape (B, 1, F, T).
        - t (torch.Tensor): The time step, of shape (B,).

        Returns:
        - torch.Tensor: The computed score or the predicted clean data `x_hat`,
    ...
    def get_pc_sampler(self, predictor_name, corrector_name, y, N=None, minibatch=None, **kwargs):
        ...
        - y (torch.Tensor): The noisy input spectrogram of shape (B, 1, F, T).
        - N (int, optional): The number of discretization steps. Defaults to `self.sde.N`.
        - minibatch (int, optional): The size of minibatches for batched sampling. Defaults to None.
        - **kwargs: Additional keyword arguments for the sampler.

        Returns:
        - function: A sampling function that returns the enhanced sample and the number of function evaluations.
        """
    ...
    def get_sb_sampler(self, sde, y, sampler_type="ode", N=None, **kwargs):
        ...
        Parameters:
        - sde: The SDE object for the Schrödinger bridge.
        - y (torch.Tensor): The noisy input spectrogram of shape (B, 1, F, T).
        - sampler_type (str, optional): The type of sampler to use ("ode" or "pc"). Defaults to "ode".
        - N (int, optional): The number of discretization steps. Defaults to `sde.N`.
        - **kwargs: Additional keyword arguments for the sampler.
    ...
    def enhance(self, y, sampler_type="pc", predictor="reverse_diffusion",
        ...
        **kwargs
        ...
        Parameters:
        - y (torch.Tensor): The noisy input spectrogram of shape 
        (B, 1, F, T).
        - sampler_type (str, optional): The type of sampler to use, e.g. "pc" or "ode". 
        Defaults to "pc".
        - predictor (str, optional): The predictor method used in the sampler, 
        e.g. "reverse_diffusion". Defaults to "reverse_diffusion".
        - corrector (str, optional): The corrector method used in the sampler, e.g. "ald". 
        Defaults to "ald".
        - N (int, optional): Number of discretization steps for the SDE solver. Defaults to 30.
        - corrector_steps (int, optional): Number of corrector steps per iteration. 
        Defaults to 1.
        - snr (float, optional): Step-size adaptation factor for the sampler. Defaults to 0.5.
        - timeit (bool, optional): If True, measure the runtime for enhancement. Defaults to False.
        - **kwargs: Additional keyword arguments passed to the sampler.

        Returns:
        - sample (torch.Tensor): The sampled (enhanced) output from the model. Retains
    ...
    def compute_loss(self, forward_out, x_t, z, t, mean, x, reduction="mean", to_audio_func=None):
        """
        ...
        Parameters:
        - forward_out (torch.Tensor): Predicted output from the score model of shape (B, 1, F, T).
        - x_t (torch.Tensor): Noisy input signal at time t in the spectrogram domain of shape (B, 1, F, T).
        - z (torch.Tensor): Noise or perturbation tensor of shape (B, 1, F, T).
        - t (torch.Tensor): Time-step tensor for the diffusion process of shape (B,).
        - mean (torch.Tensor): Estimated mean (clean signal) from the model of shape (B, 1, F, T).
        - x (torch.Tensor): Ground-truth clean signal in the spectrogram domain of shape (B, 1, F, T).
        - reduction (str): Specifies the reduction to apply to the per-sample loss. "mean" returns a scalar loss,
        whereas "none" returns a tensor of shape (B,) with the loss for each sample.
        - to_audio_func (callable): Function that converts spectrogram data to time-domain audio. This must be provided
        when using the "data_prediction" loss type.

        Returns:
        - loss (torch.Tensor): Computed loss. If reduction is "mean", the returned tensor is a scalar; if "none",
        ...
        if self.loss_type == "score_matching":
            ...
            per_sample_loss = 0.5 * torch.sum(losses.reshape(losses.shape[0], -1), dim=-1)
            ...
            if to_audio_func is None:
                raise ValueError("to_audio_func must be provided for data prediction loss")
            ...
            losses_tf = 0.5 * torch.sum(losses_tf.reshape(losses_tf.shape[0], -1), dim=-1)
            ...
            losses_l1 = 0.5 * torch.sum(losses_l1.reshape(losses_l1.shape[0], -1), dim=-1)

            if self.pesq_weight > 0.0:
                losses_pesq = self.pesq_loss(x_td, x_hat_td)
                losses_pesq = torch.mean(losses_pesq)  # Assuming pesq_loss returns per-sample losses
                per_sample_loss = losses_tf + self.l1_weight * losses_l1 + self.pesq_weight * losses_pesq
            else:
