...
try:
    ...
    print("Please install transformer from HuggingFace to use Hubert!")


class HuggingFaceHubert(nn.Module):
    ...
    ---------
    source : str
        HuggingFaceHubert hub name: e.g "facebook/hubert-base-ls960"
    save_path : str
    ...
    def __init__(
        ...
        # The extractor is only used to retrieve the normalisation, The same feature extractor for Wav2vec2 is used in Hubert
        ...
        if not (pretrain):
            config = HubertConfig.from_pretrained(source, cache_dir=save_path)
        ...
        # We check if inputs need to be normalized w.r.t pretrained Hubert
