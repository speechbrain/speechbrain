...
from speechbrain.lobes.models.synthesis.dataio import (  # noqa
    audio_pipeline,  # noqa
    spectrogram,  # noqa
    resample,  # noqa
    mel_spectrogram,  # noqa
)  # noqa
...
def text_encoder(max_input_len=128, tokens=None, takes="label"):
    ...
    @sb.utils.data_pipeline.provides(
        "text_sequences", "input_lengths", "text_positions"
    )
    def f(label):
...
def trim(takes, provides, threshold=-30.0):
    """
...
def done(
    ...
    def f(target_lengths, padding=2):
        ...
        ranges = torch.cat(
            ...
            * [
                torch.arange(
                    done.size(-1), device=target_lengths.device
                ).unsqueeze(0)
            ]
        )
        offsets = done_lengths.unsqueeze(-1)
        done[ranges >= offsets] = 1.0
...
def frame_positions(
    takes="mel", provides="frame_positions", max_output_len=1024
):
    """
    ...
    def f(mel):
        ...
        return torch.arange(1, mel_len + 1, device=mel.device).expand(
            (batch_size, mel_len)
        )
...
def normalize_spectrogram(
    takes, provides, min_level_db, ref_level_db, absolute=False
):
    """
    ...
    def f(linear):
        ...
        linear_db = (
            ref_level_db * torch.log10(torch.maximum(min_level, linear))
            - ref_level_db
        )
...
def target_lengths(takes="mel_raw", provides="target_lengths"):
    ...
    def f(mel):
        mel_1dim = mel[:, 0, :] > 0.0
        lengths = (
            torch.arange(mel.size(-1), device=mel_1dim.device).expand_as(
                mel_1dim
            )
            + 1
        )
        lengths[mel_1dim == 0.0] = 0
        return lengths.max(dim=-1).values

    return f


def pad_to_length(tensor, length, value=0.0):
    """
...
def pad_spectrogram(
    takes="linear", provides="linear_pad", outputs_per_step=1, downsample_step=5
):
    """
    Pads the last dimension of a tensor to the specified length,
    at the end
    ...
    def f(tensor: torch.Tensor):
        padding = (outputs_per_step, downsample_step * outputs_per_step, 0, 0)
...
def denormalize_spectrogram(
    takes="linear", provides="linear_denorm", min_level_db=-100, ref_level_db=20
):
    """
    ...
    def f(original_spectrogram):
        x = (
            torch.clip(original_spectrogram, 0.0, 1.0) * -min_level_db
            + min_level_db
        )
        x += ref_level_db
