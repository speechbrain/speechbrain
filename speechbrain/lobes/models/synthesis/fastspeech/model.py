...
class FastSpeech(nn.Module):
    def __init__(self, pre_net_dropout,
        ...
        self.durPred = DurationPredictor(in_channels=enc_d_model, out_channels=enc_d_model)
    ...
    def forward(self, phonemes, durations=None):
        ...
        srcmask = get_key_padding_mask(phoneme_feats, pad_idx=self.padding_idx)
        attn_mask = srcmask.unsqueeze(-1).repeat(self.enc_num_head, 1, phoneme_feats.shape[1])
        phoneme_feats, memory = self.encoder(phoneme_feats, src_mask=attn_mask, src_key_padding_mask=srcmask)
