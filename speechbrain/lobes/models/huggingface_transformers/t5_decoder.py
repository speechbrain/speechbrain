...
class T5DecoderModelForDialogueUnderstanding(nn.Module):
    ...
    def __init__(self, source, save_path, freeze=False):
        ...
        if self.freeze:
            logger.warning(
                "speechbrain.lobes.models.huggingface_T5Decoder - T5 decoder is frozen."
            )
            self.model.eval()
    ...
    def _from_pretrained(self, source, config, model, save_path):
        ...
        if is_sb:
            ...
            self._load_sb_pretrained_t5_parameters(ckpt_full_path)
        else:
