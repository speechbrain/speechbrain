...
class Mimi(HFTransformersInterface):
    ...
    >>> save_path = "savedir"
    >>> model = Mimi(model_hub, save_path)  # doctest: +SKIP
    >>> audio = torch.randn(4, 48000)  # doctest: +SKIP
    >>> length = torch.tensor([1.0, 0.5, 0.75, 1.0])  # doctest: +SKIP
    >>> tokens, emb = model.encode(audio, length)  # doctest: +SKIP
    >>> tokens.shape  # doctest: +SKIP
    torch.Size([4, 8, 25])
    >>> emb.shape  # doctest: +SKIP
    torch.Size([4, 8, 25, 256])
    >>> rec = model.decode(tokens, length)  # doctest: +SKIP
