...
class BEATs(nn.Module):
    def __init__(
        ...
        """
        Initializes the BEATs model.

        Parameters
        ----------
        ckp_path : str, optional
            Path to the checkpoint file. If None, the model will be initialized without loading weights.
        freeze : bool, optional (default: False)
            If True, the model is set to evaluation mode and parameters are frozen.
        ...
        ------
        FileNotFoundError
            If the provided `ckp_path` does not exist.

        Example
        -------
        >>> audio = torch.randn(4, 10000)
        >>> length = torch.tensor([1.0, .5, .75, 1.0])
        >>> ckp_path = "BEATs_iter3.pt"
        >>> model = BEATs(ckp_path)
        ...
        # Initialize configuration and load checkpoint if available
        ...
        # Model attributes
        self.freeze = freeze
        self.output_all_hiddens = output_all_hiddens  # Outputs all transformer layers if True
        ...
        assert not self.cfg.deep_norm or not self.cfg.layer_norm_first
        ...
        # Set model to evaluation mode if frozen
        if self.freeze:
            self.eval()
    ...
    def extract_features(
        ...
        if length is not None:
            ...
            length * max_len, max_len, device=audio.device).bool()
...
model2 = BEATs()
outputs2 = model2.extract_features(audio, length)
