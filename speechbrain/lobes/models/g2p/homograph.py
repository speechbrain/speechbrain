...
class SubsequenceLoss(nn.Module):
    ...
    def forward(self, phns, phn_lens, p_seq, subsequence_phn_start, subsequence_phn_end):
        p_seq_subsequence, phns_subsequence, subsequence_lengths = self._subsequence_extractor(
...
class SubsequenceExtractor:
    ...
    def __call__(self, phns, phn_lens, p_seq, subsequence_phn_start, subsequence_phn_end):
        return self.extract_seq(phns, phn_lens, p_seq, subsequence_phn_start, subsequence_phn_end)

    def extract_seq(self, phns, phn_lens, p_seq, subsequence_phn_start, subsequence_phn_end):
        ...
        gap = ((subsequence_phn_end + longest_subsequence).max() - phn_lens.max()).int().item()
        if gap > 0:
            phns = torch.nn.functional.pad(
                phns, (0, gap))
        p_seq_pad = (gap + longest_subsequence).item()
        p_seq_edge = p_seq.size(1)
        p_seq = torch.nn.functional.pad(
            p_seq, (0, 0, 0, p_seq_pad))
        ...
        range_phns = torch.arange(
            phns.size(1), device=phns.device).expand_as(phns)
        range_phns_subsequence = torch.arange(longest_subsequence, device=phns.device).expand(
            phns.size(0), longest_subsequence)
        match = (
            (range_phns >= subsequence_phn_start_unsq)
        ...
        phns_subsequence[range_phns_subsequence >= subsequence_lengths.unsqueeze(-1)] = 0.
        ...
        p_seq_subsequence_range = (
            torch.arange(p_seq_subsequence.size(1), device=p_seq_subsequence.device)
                .unsqueeze(0)
                .unsqueeze(-1)
                .expand_as(p_seq_subsequence))
        p_seq_subsequence[p_seq_subsequence_range >= (word_end_unsq - word_start_unsq)] = 0.
        return (
            p_seq_subsequence,
            phns_subsequence,
    ...
    def _get_word_boundaries(self, p_seq, word_indexes, edge):
        ...
        # word boundaries
        words_range  = (
            torch.arange(words.size(-1), device=words.device)
            .expand_as(words))
    ...
    def extract_hyps(
        ...
        separator_indexes = [
            [-1] + [idx for idx, phn in enumerate(item_hyps) if phn == self.word_separator] + [None]
            for item_hyps in hyps
        ]
        return [
            self._extract_hyp_word(
                item_hyps, item_separtaor_indexes, word_index)
            for item_hyps, item_separtaor_indexes, word_index
            in zip(hyps, separator_indexes, target_word_indexes)
        ]
