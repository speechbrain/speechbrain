...
class SBTransformerBlockRP(nn.Module):
    ...
    def forward(self, x, pos_embs, init_params=False):
        ...
        return self.mdl(x, pos_embs, init_params=init_params)
...
#            norm: gln = "Global Norm", cln = "Cumulative Norm", ln = "Layer Norm"
    ...
    def forward(self, x,  pos_embs, init_params=True):
        """
           x: [B, N, K, S]
           out: [Spks, B, N, K, S]
        """
        ...
        intra = self.intra_mdl(intra, pos_embs, init_params=init_params)
        ...
        # [BK, S, H]
        inter = self.inter_mdl(inter, pos_embs, init_params=init_params)
...
class Dual_Path_Model(nn.Module):
    def __init__(
        ...
        norm="ln",
        K=200,
        num_spks=2, position="absolute", position_max_len=4000
    ):
        ...
        self.norm = select_norm(norm, in_channels, 3)
        self.conv1d = nn.Conv1d(in_channels, out_channels, 1, bias=False)
        assert position in ["absolute", "relative", "none"]

        if position == "absolute":
            self.pos_encoding = PositionalEncoding(position_max_len)
        elif position == "relative":
            self.pos_enc_intra = PositionalEncoding(self.K)
            self.pos_enc_inter = PositionalEncoding(position_max_len)
        else:
            pass
    ...
    def forward(self, x, init_params=True):
        ...
        if self.position == "absolute":
            # add sinusoidal pos encoding here
            x = self.pos_encoding(x) + x*(x.size(1)**0.5)
        ...
        # [B, N*spks, K, S]
        for i in range(self.num_layers):
            x = self.dual_mdl[i](x, pos_embs, init_params=init_params)
