...
class MusicTransformer(nn.Module):
    ...
    def forward(self, src):
        ...
        pos_encod  = src + self.positional_encoding(src)

        output, _ = self.encoder(pos_encod)
...
class TransformerEncoder(nn.Module):
    ...
    max_seq_length: int
        The dimension of the maximum sequence length of the input.
    kdim : int
...
class TransformerEncoderLayer(nn.Module):
    ...
    erdim: int
        The dimension of the relative positional encoding(Er) matrix.
    kdim : int
    ...
    def __init__(
        ...
        self.self_att = sb.nnet.attention.MultiheadRelativeAttention(
            num_heads=nhead, embed_dim=d_model, erdim=erdim, dropout=dropout, kdim=kdim, vdim=vdim,
        )
