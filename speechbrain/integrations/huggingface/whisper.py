...
# Non-speech tokens for monolingual (English-only) Whisper models.
NON_SPEECH_TOKENS = [
    1, 2, 7, 8, 9, 10, 14, 25,
...
# Non-speech tokens for multilingual Whisper models.
...
class Whisper(HFTransformersInterface):
    ...
    def get_suppress_tokens(self):
        """Returns the list of tokens to suppress based on model type."""
        if self.is_multilingual:
            return tuple(sorted(NON_SPEECH_TOKENS_MULTI))
        return tuple(sorted(NON_SPEECH_TOKENS))
