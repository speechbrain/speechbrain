...
class BaseRescorerInterface(BaseScorerInterface):
    ...
    This scorer allows one to rescore the top-K candidates with a LM that
    has different tokens than the ASR model.

    In this approach, a neural network is utilized to provide scores for candidate
    text transcripts. A candidate refers to the predicted transcript generated by the
    decoder of the ASR model. The beam search decoding process generates a set of top K
    candidates, where K represents the beam width. These candidates are then passed to a
    neural language model to rank them. The ranking is accomplished by a language model
    that assigns a score to each candidate.
        ...
        for k, impl in self.rescorers.items():
            ...
            for i in range(len(new_scores)):
                for j in range(len(new_scores[i])):
                    new_scores[i][j] += self.weights[k] * scores[index_scores]
                    index_scores += 1
