...
class TransducerBeamSearcher:
        ...
        for i_batch in range(tn_output.size(0)):
            ...
            for t_step in range(tn_output.size(1)):
                ...
                while True:
                    ...
                    if self.lm_weight > 0:
                        log_probs_lm, hidden_lm = self.lm_forward_step(
    ...
    def joint_forward_step(self, h_i, out_PN):
        """
        ...
        with torch.no_grad():
            ...
            out = _forward_after_joint(out, self.classifier_network)
