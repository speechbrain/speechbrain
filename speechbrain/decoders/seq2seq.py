    ...
    def __init__(
        ...
        if self.ctc_weight > 0.0:
            if len({self.bos_index, self.eos_index, self.blank_index}) < 3:
                raise ValueError(
                    "To perform joint ATT/CTC decoding or ctc decoding, set blank, eos and bos to different indexes."
                )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.ctc_weight > 0.0 and self.ctc_fc is None:
            raise ValueError(
                "To perform joint ATT/CTC decoding or ctc decoding, ctc_fc is required."
            )
