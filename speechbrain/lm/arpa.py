...
Authors
 * Aku Rouhe 2020
 * k2/kaldi team (https://github.com/k2-fsa/icefall/blob/master/icefall/shared/make_kn_lm.py)
"""
import collections
...
# The flowing code is copied from k2-fsa/icefall and is an implementation of computing Kneser-Ney smoothed language model
...
def Make(input_text_file, output_arpa_dir, ngram_order=3):
    """
    ...
    logger.info(f"Making arpa {ngram_order}gram kneser-ney language model from: {input_text_file}")
    ngram_counts.add_raw_counts_from_file(input_text_file)
    ngram_counts.cal_discounting_constants()
    ngram_counts.cal_f()
    ngram_counts.cal_bow()
    with open(os.path.join(output_arpa_dir, f"{ngram_order}gram.arpa"), "w", encoding=default_encoding) as f:
