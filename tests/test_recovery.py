...
def test_recoverer(tmpdir_factory):
    ...
    assert recoverable.param.data == torch.tensor([2.])
    assert other.param.data == torch.tensor([10.])
    other_result = other(10.)
    # Check that parameter for other has now been loaded:
    assert other.param.data == torch.tensor([2.])
    # And again we should have loaded that before computations:
    assert other_result == 20.
    ...
    other_result = other(10.)
    assert other.param.data == torch.tensor([10.])
    assert other_result == 100.

    # Test saving names checkpoints with meta info, and custom filter
    epoch_ckpt = recoverer.save_checkpoint(name = "ep1", meta = {"loss": 2.})
