...
def _batch_fake_probs(indexes, count):
    ...
    for batch_idx in range(len(indexes)):
        for item_idx in range(indexes.size(1)):
            p_seq[batch_idx, item_idx, :] = _fake_probs(indexes[batch_idx, item_idx], count)
    return p_seq
...
def test_subsequence_loss():
    ...
    loss_value = loss.forward(
        phns, phn_lens, p_seq.log(), start, end)
    loss_value_ref = nll_loss(
        word_p_seq.log(), word_phns_ref, word_lengths)
...
def test_extract_hyps():
    ...
    hyps = [[1, 2, 3, 2, 0, 3, 1, 2, 1, 0, 3, 1, 2],
    ...
    subsequence_hyps = extractor.extract_hyps(
        ref_seq=phns,
        hyps=hyps,
        subsequence_phn_start=subsequence_phn_start)
