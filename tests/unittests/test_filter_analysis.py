...
def test_simple_filter_stacks():
    assert (
        ...
        .with_on_top(FilterProperties(window_size=3, stride=2))
        ==
    ...
    assert (
        ...
        .with_on_top(FilterProperties(window_size=3, stride=1))
        ==
...
def test_causal_filter_properties():
    assert (
        FilterProperties(3, 1, causal=True).get_noncausal_equivalent()
        == FilterProperties(5, 1)
    )
