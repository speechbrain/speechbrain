...
def test_audio_io_roundtrip_wav(tmpdir):
    ...
    # Check sample rate
    assert loaded_sr == sample_rate, f"Expected sample rate {sample_rate}, got {loaded_sr}"
    
    # Check shape
    assert loaded_waveform.shape[0] == 1, f"Expected 1 channel, got {loaded_waveform.shape[0]}"
    
    # Check values are close (allow for encoding/decoding differences)
    assert torch.allclose(loaded_waveform, waveform, atol=1e-3), "Waveforms don't match"
...
def test_audio_io_roundtrip_flac(tmpdir):
    ...
    sample_rate = 22050
    waveform = torch.randn(1, 22050)  # 1 second of random noise
...
def test_audio_io_info(tmpdir):
    ...
    waveform = torch.randn(1, num_frames)
    ...
    # Check metadata
    assert info.sample_rate == sample_rate, f"Expected sample rate {sample_rate}, got {info.sample_rate}"
    assert info.frames == num_frames, f"Expected {num_frames} frames, got {info.frames}"
    ...
    assert abs(info.duration - duration) < 0.01, f"Expected duration ~{duration}s, got {info.duration}s"
...
def test_audio_io_load_channels_first(tmpdir):
    ...
    # Create stereo audio
    sample_rate = 16000
    waveform = torch.randn(2, 8000)  # (channels, frames)
    ...
    assert loaded_cf.shape == (8000, 2), f"Expected (8000, 2), got {loaded_cf.shape}"
...
def test_audio_io_load_always_2d(tmpdir):
    ...
    # Create mono audio
    sample_rate = 16000
    waveform = torch.randn(16000)  # 1D mono
    ...
    assert loaded.shape == (1, 16000), f"Expected (1, 16000), got {loaded.shape}"
    
    # Load with always_2d=True, channels_first=False
    loaded_cf, sr = audio_io.load(audio_path, channels_first=False, always_2d=True)
    assert loaded_cf.shape == (16000, 1), f"Expected (16000, 1), got {loaded_cf.shape}"
...
def test_audio_io_save_shapes(tmpdir):
    ...
    waveform_2d = torch.randn(1, 8000)
    ...
    # Test 3D batched input (batch, channels, frames)
    waveform_3d = torch.randn(2, 1, 8000)  # batch of 2
    path_3d = os.path.join(tmpdir, "batch.wav")
    audio_io.save(path_3d, waveform_3d, sample_rate)  # Should save first batch element
    loaded_3d, _ = audio_io.load(path_3d, channels_first=True, always_2d=True)
    assert loaded_3d.shape == (1, 8000)
...
def test_audio_io_save_stereo(tmpdir):
    ...
    sample_rate = 16000
    waveform = torch.randn(2, 8000)  # Stereo (2 channels)
    
    audio_path = os.path.join(tmpdir, "stereo.wav")
    audio_io.save(audio_path, waveform, sample_rate)
    
    loaded, sr = audio_io.load(audio_path, channels_first=True)
    assert loaded.shape == (2, 8000), f"Expected stereo (2, 8000), got {loaded.shape}"
...
def test_audio_io_numpy_input(tmpdir):
    ...
    sample_rate = 16000
    waveform_np = np.random.randn(1, 8000).astype(np.float32)
