...
def test_denoising_diffusion_distort():
    ...
    x_noisy = diffusion.distort(x, noise=noise, timesteps=timesteps)
    assert x_noisy[0].allclose(torch.tensor([1.05, 2.05]), atol=0.01)
    assert x_noisy[1].allclose(torch.tensor([9.103, 9.913]), atol=0.001)
    ...
    assert x_noisy[2].mean() < 0.5


class DummyModel(nn.Module):
    def forward(self, x, timesteps):
        return x * 1.001 + timesteps * 0.00001
...
def test_denoising_diffusion_sample():
    ...
    sample_ref = torch.tensor(
        [[0.0167, -0.5407], [2.1752, 0.4488], [0.4631, -0.3857]]
    )
