...
class SLU(sb.Brain):
    def compute_forward(self, x, y, stage):
        ...
        with torch.no_grad():
            ...
            ASR_encoder_out = self.modules.asr_enc(feats.detach())
    ...
    def load_pretrained_asr(self):
        """Downaloads and loads the pre-trained ASR model."""

        save_model_path = hparams["output_folder"] + "/save/asr_enc.ckpt"

        if "http" in hparams["asr_enc_file"]:
            download_file(hparams["asr_enc_file"], save_model_path)
        self.modules.asr_enc.load_state_dict(
        ...
        self.modules.asr_enc.eval()
        for p in self.modules.asr_enc.parameters():
