...
def eval_voxceleb_gender(model, hparams):
    ...
    for key in data.keys():
        ...
        if "female" in data[key]["output"]:
            ...
            real_label = 1
        metric.append([key], torch.tensor([pred_label]), torch.tensor([real_label]))
...
def eval_esc50(model, hparams):
    ...
    for key in data.keys():
        ...
        messages = [
            {"role": "system", "content": "You are a sound classifier who classifies a sound description into a label."},
        ...
        output = tokenizer.decode(response, skip_special_tokens=True).replace("assistant\n\n", "")
        output = output.lower()
...
if __name__ == "__main__":
    ...
    with open(hparams["eval_log"], "w") as f:
        if hparams["eval_iemocap_emo_json"] is not None:
            f.write(f"IEMOCAP emotion recognition (Acc) : {eval_iemocap_emo(ltu_as, hparams)}\n")
        if hparams["eval_voxceleb_gender_json"] is not None:
            f.write(f"Voxceleb2 test gender classification (F1) : {eval_voxceleb_gender(ltu_as, hparams)}\n")
        if hparams["eval_voxceleb_age_json"] is not None:
            f.write(f"Voxceleb2 test age prediction (MAE) : {eval_voxceleb_age(ltu_as, hparams)}\n")
        if hparams["eval_librispeech_asr_json"] is not None:
            f.write(f"Librispeech test clean ASR (WER) : {eval_librispeech_asr(ltu_as, hparams)}\n")
        if hparams["eval_esc50_json"] is not None:
            f.write(f"ESC50 audio classification (Acc) : {eval_esc50(ltu_as, hparams)}\n")
