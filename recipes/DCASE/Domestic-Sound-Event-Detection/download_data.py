...
def download_file(result_dir, filename):
    ...
    # multiprocessing can give this error
        ...
        # LOG.info(filename)
        # LOG.info(str(e))
        return [filename, str(e)]  # "Index Error"]


def download(
    filenames, result_dir, n_jobs=1, chunk_size=10, base_dir_missing_files=".."
):
    ...
    existing_files = [
        ...
        for fpath in glob.glob(os.path.join(result_dir, "*"))
    ]
    ...
    try:
        ...
        if not missing_files.empty:
            ...
            missing_files.to_csv(
                os.path.join(
                    base_dir_missing_files, result_dir.split("\\")[-1] + ".tsv"
                ),
                index=False,
                sep="\t",
            )
...
def run_download(hparams):
    ...
    test = os.path.join(
        dataset_folder, "metadata", "validation", "validation.tsv"
    )
    ...
    train_unlabel_in_domain = os.path.join(
        dataset_folder, "metadata", "train", "unlabel_in_domain.tsv"
    )
    result_dir = os.path.join(
        dataset_folder, "audio", "train", "unlabel_in_domain"
    )
    # read metadata file and get only one filename once
    df = pd.read_csv(train_unlabel_in_domain, header=0, sep="\t")
    filenames_unlabel_in_domain = df["filename"].drop_duplicates()
    download(
        filenames_unlabel_in_domain,
        result_dir,
        n_jobs=N_JOBS,
        chunk_size=CHUNK_SIZE,
        base_dir_missing_files=base_missing_files_folder,
