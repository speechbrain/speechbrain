...
def get_evaluation_metrics(
    reference_list, prediction_list, class_list, f_measure_only=False
):
    ...
    segment_based_metrics = sed_eval.sound_event.SegmentBasedMetrics(
        event_label_list=class_list, time_resolution=1.0
    )
    ...
    seg_f_measure = segment_based_metrics.results_class_wise_average_metrics()[
        "f_measure"
    ]["f_measure"]
    event_f_measure = event_based_metrics.results_class_wise_average_metrics()[
        "f_measure"
    ]["f_measure"]
...
def get_batch_predictions(
    strong_pred, weak_pred, filenames, decoder, append_list
):
    ...
    None
    """
    depooling = hparams["pooling_time_ratio"] / (
        hparams["sample_rate"] / hparams["hop_length"]
    )
    ...
    weak = dcase_util.data.ProbabilityEncoder().binarization(
        weak_pred,
    ...
    for sample, filename in zip(strong_pred, filenames):
        ...
        for i, window in enumerate(window_sizes):
            sample[:, i] = scipy.ndimage.filters.median_filter(
                sample[:, i], (window,)
            )

        sample = dcase_util.data.ProbabilityEncoder().binarization(
            sample, binarization_type="global_threshold", threshold=0.5
        )
...
class EventBrain(sb.Brain):
    ...
    def prepare_features(self, wavs, stage, ema):
        ...
        """
        wavs, lens = wavs
        if ema is False and stage == sb.Stage.TRAIN:
            wavs_noise = self.modules.env_corrupt_5(wavs, lens)
            feat = self.hparams.compute_features(
                wavs_noise
            )  # , self.hparams.STFTArgs, self.hparams.FBArgs)
        elif ema is True and stage == sb.Stage.TRAIN:
    ...
    def compute_objectives(
        ...
        if weak_mask is not None:
            weak_class_loss = self.hparams.loss(
                weak_pred[weak_mask], target_weak[weak_mask]
            )
            if weak_pred_ema is not None:
                weak_ema_class_loss = self.hparams.loss(
                    weak_pred_ema[weak_mask], target_weak[weak_mask]
                )
                self.hparams.weak_ema_loss = self.update_average(
                    weak_ema_class_loss, self.hparams.weak_ema_loss
                )

            self.hparams.weak_loss = self.update_average(
                weak_class_loss, self.hparams.weak_loss
            )

            loss += weak_class_loss

        if strong_mask is not None:
            strong_class_loss = self.hparams.loss(
                strong_pred[strong_mask], target_strong[strong_mask]
            )
            if strong_pred_ema is not None:
                # strong_ema_class_loss = self.hparams.loss(
                #     strong_pred_ema[strong_mask], target_strong[strong_mask]
                # )
                self.hparams.strong_ema_loss = self.update_average(
                    weak_ema_class_loss, self.hparams.strong_ema_loss
                )

            self.hparams.strong_loss = self.update_average(
                strong_class_loss, self.hparams.strong_loss
            )
        ...
        if self.hparams.Unlabel == 1 and stage == sb.Stage.TRAIN:
            ...
            consistency_loss_weak = (
                consistency_cost
                * self.hparams.consistency_loss(weak_pred, weak_pred_ema)
            )
            self.hparams.consistency_loss_weak = self.update_average(
                consistency_loss_weak, self.hparams.consistency_loss_weak
            )
            ...
            consistency_loss_strong = (
                consistency_cost
                * self.hparams.consistency_loss(strong_pred, strong_pred_ema)
            )
            self.hparams.consistency_loss_strong = self.update_average(
                consistency_loss_strong, self.hparams.consistency_loss_strong
            )
    ...
    def fit_batch(self, batch):
        ...
        strong, weak = self.compute_forward(
            batch, ema=False, stage=sb.Stage.TRAIN
        )
        if self.hparams.Unlabel == 1:
            # forward pass for MT model
            strong_ema, weak_ema = self.compute_forward(
                batch, ema=True, stage=sb.Stage.TRAIN
            )
            # computing ramp-up value
            rampup_length = (
                self.hparams.len_train_loader
        ...
        if self.hparams.Unlabel == 1:
            update_ema_variables(
                self.modules.crdnn, self.modules.crdnn_ema, 0.999, self.step - 1
            )
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            self.hparams.train_consistency = (
                self.hparams.consistency_loss_strong
                + self.hparams.consistency_loss_weak
            )

        if stage == sb.Stage.VALID:

            seg_score, event_score = get_evaluation_metrics(
                self.hparams.ref_event_list,
                self.hparams.est_event_list,
            ...
            stats = {
                ...
                "consistency_loss": self.hparams.consistency_loss_strong
                + self.hparams.consistency_loss_weak,
            ...
            self.hparams.train_logger.log_stats(
                ...
                train_stats={
                    "total_loss": self.hparams.train_total_loss,
            ...
            self.checkpointer.save_and_keep_only(
                meta=stats, max_keys=["Event f_score"]
            )
    ...
    def evaluate_batch(self, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            mh = ManyHotEncoder(self.hparams.classes)
            self.hparams.est_event_list = get_batch_predictions(
                strong,
                weak,
                filenames,
                mh.decode_strong,
...
if __name__ == "__main__":
    ...
    # set the device
    if hparams["GPUAvailable"] is False:
        run_opts["device"] = "cpu"

    # Download the data if flag is up
    if hparams["download"] == 1:
        run_download(hparams)
    ...
    (
        ...
        batch_sampler_2_toy,
        batch_sampler_3_toy,
    ) = data_label_pipeline.dataio_prep(hparams)
