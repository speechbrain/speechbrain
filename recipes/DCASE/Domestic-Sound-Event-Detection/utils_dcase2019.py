...
class ManyHotEncoder:
    ...
    def decode_strong(
        self, encoded_labels, filename, depooling, append_list=False
    ):
        ...
        for i, label_column in enumerate(encoded_labels.T):
            event_dict = {}
            change_indices = DecisionEncoder().find_contiguous_regions(
                label_column
            )
...
def extract_ground_truth_list(event_label, filename):
    ...
    for event, info in event_label.items():
        ev_dict = {"event_label": "", "onset": "", "offset": "", "filename": ""}
