...
class DiffusionBrain(sb.Brain):
    ...
    def prepare_features(self, batch, stage):
        ...
        # Global Norm
        feats = self.modules.global_norm(feats_raw, lens)
        ...
        if self.hparams.use_tensorboard:
            ...
            if self.hparams.use_tensorboard:                
                ...
                for sample in samples:
                    self.hparams.tensorboard_train_logger.log_figure("spectrogram", sample)
                if wav is not None:
                    for wav_sample in wav:
                        self.hparams.tensorboard_train_logger.writer.add_audio("audio", wav_sample)
