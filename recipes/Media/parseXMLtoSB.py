                ...
                if speaker == speaker_id:
                    for n in range(len(sentences[speaker])):
                        ...
                        duration = str(
                            "%.2f"
                            % (
                                float(sentences[speaker][n][3])
                                - float(sentences[speaker][n][2])
                            )
                        )
                        if (
                            float(wav_duration)
                            ...
                            data.append(
                                [
                                    IDs[n],
                                    duration,
                                    sentences[speaker][n][2],
                                    sentences[speaker][n][3],
                                    wav,
                                    "wav",
                                    speaker_name,
                                    "string",
                                    sentences[speaker][n][0],
                                    "string",
                                    sentences[speaker][n][1],
                                    "string",
                                ]
                            )
    return data
...
def parse_sentences(
    ...
    (
        ...
        has_speech,
        concept_added,
    ) = init_parsing(time_beg, time_end, speakers, current_speaker)
    ...
    for node in nodes:
        ...
        if node.nodeName == "SemDebut":
            ...
            if (
                filename + "#" + identifiant in specifiers
                and method == "relax"
                and concept[current_speaker][
                    -len(specifiers[filename + "#" + identifiant]) :
                ]
                == specifiers[filename + "#" + identifiant]
            ):
                concept[current_speaker] = concept[current_speaker][
                    : -len(specifiers[filename + "#" + identifiant])
                ]
        ...
        if (
            ...
            if concept[current_speaker] != "null" and not (
                ...
                sentences[current_speaker][n[current_speaker]][1] += (
                    "<" + concept[current_speaker] + "> _ "
                )
            ...
            sentence = " ".join(
                sentence.split()
            )  # Remove double spaces made from the first replacement
        ...
        if node.nodeName == "SemFin":
            ...
            concept[
                current_speaker
            ] = "null"  # Indicate there is no currently open concept

        if node.nodeName == "Sync":
            for speaker in speakers:
                # If the segment has no speech yet
                if not (has_speech[speaker]):
                    sentences[speaker][n[speaker]][2] = node.getAttribute(
                        "time"
                    )  # Change time_beg for the last segment of each speaker
                # If the segment has speech
                else:
                    sentences[speaker][n[speaker]][3] = node.getAttribute(
                        "time"
                    )  # Change time_end for the last segment of each speaker
                    sentences[speaker].append(
                        ["", "", sentences[speaker][n[speaker]][3], time_end]
                    )  # Create new segment for each speaker
...
def init_parsing(time_beg, time_end, speakers, current_speaker):
    currently_cut_concept = {}
    currently_open_concept = {}
    has_speech = {}
    sentences = {}
...
def normalize_sentences(sentences, filename):
    for speaker in sentences:
        for n in range(len(sentences[speaker])):
            if sentences[speaker][n][0] != "":
                sentence = sentences[speaker][n][0][:-1]  # Remove last ' '
                sentences[speaker][n][0] = normalize_sentence(
                    sentence, filename
                )
                sentence = sentences[speaker][n][1][:-3]  # Remove last ' _ '
                sentences[speaker][n][1] = normalize_sentence(
                    sentence, filename
                )
            else:
                del sentences[speaker][n]  # Usefull for last appended segment
    return sentences
...
def normalize_sentence(sentence, filename):
    sentence = re.sub(
        r"\(.*?\)", "", sentence
    )  # Remove round brackets and all in between
    sentence = re.sub(
        r"[^\w\s'-><_]", "", sentence
    ...
    for index in range(len(words)):
        if words[index].isdigit():
            words[index] = num2words.num2words(
                words[index]
...
def get_corpora(data_folder):
    ...
    proc = subprocess.Popen(
        "egrep -a '<dialogue' "
        + data_folder
        + "/E0024/MEDIA1FR_00/MEDIA1FR/DATA/media_lot*.xml | cut -d' ' -f2 | cut -d'"
        + '"'
        + "' -f2",
        stdout=subprocess.PIPE,
        shell=True,
    ...
    proc = subprocess.Popen(
        "egrep -a '<dialogue' "
        + data_folder
        + "/E0024/MEDIA1FR_00/MEDIA1FR/DATA/media_testHC_a_blanc.xml | cut -d' ' -f2 | cut -d'"
        + '"'
        + "' -f2",
        stdout=subprocess.PIPE,
        shell=True,
    ...
    proc = subprocess.Popen(
        "egrep -a '<dialogue' "
        + data_folder
        + "/E0024/MEDIA1FR_00/MEDIA1FR/DATA/media_testHC.xml | cut -d' ' -f2 | cut -d'"
        + '"'
        + "' -f2",
        stdout=subprocess.PIPE,
        shell=True,
    ...
    for speaker in root.getElementsByTagName("Speaker"):
        if speaker.getAttribute("name")[0] == "s":
            return (
                ...
                "_".join(speaker.getAttribute("name").split("#")).replace(
                    "-", "_"
                ),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "wav_folder", type=str, help="Path where the wavs will be stored."
    )
    parser.add_argument(
        "csv_folder", type=str, help="Path where the csv will be stored."
    )
    parser.add_argument(
        "-w",
        "--skip_wav",
