    ...
    def compute_objectives(
        ...
        if "supervision_segments" not in batch:
            batch["supervision_segments"], batch["wrd"] = self.get_supervision_segments(
                n_frames, batch['seq_ids'], batch['wrd']
            )
        wrd: List[str] = batch["wrd"]
        # print(f"supervision_segments: {supervision_segments}")
        assert isinstance(wrd, list) and isinstance(wrd[0], str), wrd and len(wrd) == len(ids)
        decoding_graph = self.graph_compiler.compile(wrd)
        dense_fsa_vec = k2.DenseFsaVec(
            p_ctc,
            batch["supervision_segments"],
            allow_truncate=self.subsampling_factor - 1,
        )
        # wrd_lens = torch.tensor([len(self.graph_compiler.tokenize(w)) for w in wrd], device=p_ctc.device)
        loss = k2.ctc_loss(
            decoding_graph=decoding_graph,
            dense_fsa_vec=dense_fsa_vec,
            output_beam=self.hparams.output_beam,
            reduction='sum',  # TODO: May need to change with CL
            # reduction='mean',  # TODO: When using `mean` we need to define token_lens
            # target_lengths=wrd_lens,
            use_double_scores=True,
        )
        loss = loss / batch["supervision_segments"][:, 2].sum()
        # logger.info(f"{batch=}\n{loss.item()=}")
