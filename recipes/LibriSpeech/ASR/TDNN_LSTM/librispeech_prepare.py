...
Minor changes by Georgios Karakasidis to accommodate for musan
augmentation (instead of doing it on the fly and changing the
batch size).
"""
...
def prepare_librispeech(
    ...
    corrupt_models : dict[str, EnvCorrupt]
        If you want to perform MUSAN augmentation then you need to provide a 
        "corrupt_model" key with an instance of speechbrain.augment.EnvCorrupt 
        class. 
    corrupt_prob: float
        If you want to perform MUSAN augmentation you also need to provide the 
        probability of applying the augmentation.
        Example: "corrupt_models": {
                "noise": EnvCorrupt(noise_csv='pathto/noise/csv', noise_prob=1.0), 
                "music": EnvCorrupt(noise_csv='pathto/music/csv', noise_prob=1.0)
            }, 
            "aug_prob": 0.4
    ...
    for split_index in range(len(splits)):
        ...
        sequence_idx = create_csv(
            save_folder, wav_lst, text_dict, split, n_sentences,
            corrupt_models=corrupt_models, corrupt_prob=corrupt_prob,
...
def create_csv(
    ...
    corrupt_models: Optional[Dict[str, EnvCorrupt]] = None,
    sequence_idx: int = 0
):
    """
    ...
    corrupt_prob : float, optional
        The probability of augmenting a sentence. If none is given, no
        augmentation is performed.
    corrupt_models : Dict[str, EnvCorrupt], optional
        The model used to corrupt the audio. If none is given, no augmentation
        is performed. If provided, then it must be an instance of EnvCorrupt.
    sequence_idx : int, optional
        The index of the current sequence. Used when generating csvs for 
        multiple sequences.
    Returns
    -------
    None
    ...
    if os.path.exists(csv_file):
        ...
        # read the csv file and set sequence_idx to the last line + 1
        with open(csv_file, "r") as f:
            lines = f.readlines()
            sequence_idx = int(lines[-1].split(",")[0]) + 1
        return sequence_idx
    ...
    # csv_lines = [["ID", "string_identifier", "duration", "wav", "spk_id", "wrd"]]
    csv_lines = [["ID", "string_identifier", "duration", "wav", "spk_id", "wrd"]]
    ...
    for wav_file in tzip(wav_lst):
        ...
        wrds = text_dict[snt_id.split("_")[0]]
        ...
        corrupt_prob = corrupt_prob or 0.  # make sure it's not None
        # Augmenting the current file with MUSAN
        if corrupt_prob > 0. and corrupt_models is not None:
            for corruption_style in corrupt_models.keys():
                if random.random() < corrupt_prob:
                    aug_wav_suffix = f"_{corruption_style}_aug"
                    new_wav_file = os.path.join(
                        os.path.dirname(wav_file),
                        Path(wav_file).stem + aug_wav_suffix + Path(wav_file).suffix
                    )
                    if not os.path.exists(new_wav_file):
                        augment_datapoint(
                            signal, 
                            fs, 
                            corrupt_models[corruption_style], 
                            new_wav_file
                        )
                    csv_line.append([
                        str(sequence_idx),
                        snt_id + aug_wav_suffix,
                        str(duration),
                        new_wav_file,
                        spk_id,
                        str(" ".join(wrds.split("_"))),
    ...
    with open(os.path.join(save_folder, f".n_utterances_{split}.txt"), "w") as f:
        f.write(str(snt_cnt))
...
def augment_datapoint(
    signal: torch.Tensor,
    fs: int,
    corrupt_model: EnvCorrupt,
    out_wav_path: Path
) -> None:
    # corrupt the wav file
    wav_corrupted = corrupt_model(
        signal.reshape(1, -1), 
        torch.ones(1)
    )
    # save the corrupted wav file
    torchaudio.save(
        out_wav_path,
        wav_corrupted,
        fs
