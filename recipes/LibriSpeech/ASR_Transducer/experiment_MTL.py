    ...
    def compute_forward(self, x, y, stage="train", init_params=False):
        ...
        if stage == "train":
            ...
            if params.epoch_counter.current <= params.number_of_ctc_epochs:
                CTC_output = params.ctc_lin(TN_output, init_params)
                CTC_output = params.log_softmax(CTC_output)
            ...
            outputs = decode_batch(
                TN_output,
                [params.decoder_embedding, params.decoder],
                params.Tjoint,
                [params.output],
                params.blank_index,
            )
    ...
    def compute_objectives(self, predictions, targets, stage="train"):
        ...
        if stage != "train":
            ...
            if (
                ...
                ctc_predictions, predictions, lens = predictions
            else:
                predictions, lens = predictions
        transducer_loss = params.compute_cost(
            predictions,
        ...
        if (
            ...
            loss_ctc = params.ctc_cost(ctc_predictions, chars, lens, char_lens)
            loss = (
                params.ctc_weight * loss_ctc
                + (1 - params.ctc_weight) * transducer_loss
            )
        else:
