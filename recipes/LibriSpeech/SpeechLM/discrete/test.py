...
audio_tokenizer = "speech-tokenizer"

# import dac
# codec_path = "/scratch/adelmou/models/dac/weights_16khz.pth"
# codec = dac.DAC.load(codec_path)
# codec.to("cuda")
# codec.eval()
from speechtokenizer import SpeechTokenizer

config_path = '/scratch/adelmou/models/speech-tokenizer/SpeechTokenizer/speechtokenizer_hubert_avg/config.json'
ckpt_path = '/scratch/adelmou/models/speech-tokenizer/SpeechTokenizer/speechtokenizer_hubert_avg/SpeechTokenizer.pt'
codec = SpeechTokenizer.load_from_checkpoint(config_path, ckpt_path)
codec.to("cuda").eval()

prompt_audio = "/home/adelmou/proj/speechbrain/gslms/speechbrain/OVBUlHqrvK.wav"
checkpoint_dir = "/scratch/adelmou/results/speech_lm/ST_test/save/"
if audio_tokenizer == "speech-tokenizer":
    x, _ = torchaudio.load(prompt_audio)
    x = x.to("cuda").view(1, 1, -1)
    codes = codec.encode(x)  # codes: (n_q, B, T)
    codes = codes.permute(1, 0, 2)
    prompt_audio = codes[:, :hparams["num_codebooks"]]
    gen_wav = codec.decode(prompt_audio.permute(1, 0, 2))
else:
    x, _ = torchaudio.load(prompt_audio)
    x = x.to("cuda").view(1, 1, -1)
    z, codes, latents, _, _ = codec.encode(x)
    prompt_audio = codes[:, :hparams["num_codebooks"]]
    z_bdt = codec.quantizer.from_codes(prompt_audio)[0]
