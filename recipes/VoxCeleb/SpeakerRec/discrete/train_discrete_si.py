...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        _, _, pr_tokens = hparams['discrete_ssl_model'](wavs, lens,**hparams['tokenizer_config'])
        embeddings = self.modules.discrete_embedding_layer(pr_tokens)
        ...
        # Save mean average across sequence for test set
        if stage == sb.Stage.TEST:
             self.att_mean.extend(torch.mean(att_w.squeeze(-1),dim=-2).cpu().detach().numpy())
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.TEST:
            with open(hparams['layer_weight_file'], 'w') as f:
                
                # using csv.writer method from CSV package
                write = csv.writer(f)
                write.writerows(self.att_mean)
