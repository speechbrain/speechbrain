...
class ResNetBrain(sb.core.Brain):
    def compute_forward(self, x, stage="train", init_params=False):
        ...
        # ResNet + speaker classifier
        feats = feats.unsqueeze(1)
        spk_emb = params.resnet_model(feats, init_params=init_params)
        outputs = params.classifier(spk_emb, init_params)
...
# ResNet Model
modules = [params.resnet_model, params.classifier]
first_x, first_y = next(iter(train_set))
if hasattr(params, "augmentation"):
    modules.append(params.augmentation)

# Object initialization for training xvector model
xvect_brain = ResNetBrain(
