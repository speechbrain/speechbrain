...
def clean_dataframe(df):
    ...
    df.drop(['utt_id','file','lvl','channel','rir_channel', 'words', 'file', 'rir'] , axis=1, inplace=True)
...
def create_json_structure(df, data_folder):
  ...
  for session_id in session_ids:
    path = data_folder + str(session_id) + '/session_' + str(session_id)+ '_mixture.wav'
...
def create_window_splits(values, compare_list, reference_list, window_size):
  ...
  iter_values = [i for i in range(start_floor*window_size, stop_floor*window_size + 1) if i%window_size==0]
...
def create_json_dataset(dic, sample_rate, window_size):
  ...
  for sessions in sessions_iteration:
    ...
    for values in speech_timings:
      reference_list, compare_list = create_window_splits(values, compare_list, reference_list, window_size)
      unique_list, seq_timing = remove_duplicates_sort(reference_list, compare_list)
    num_speech_regions = len(unique_list)    
...
def prepare_libriparty(
    ...
    skip_prep=False):
    """
    repares the json files for the LibriParty dataset.
    ...
    df_columns = pd.DataFrame(columns=['start','stop','words','rir','utt_id', 'file','lvl','channel','rir_channel','session_id'])
    ...
    save_dataset(save_json_folder + '/val.json', valid_dataset)
