...
def swahili_prepare(data_folder):
    ...
    train_dvoice = pd.read_csv(f"{data_folder}/DVOICE/swahili/texts/train.csv", sep="\t")
    ...
    f_train_alffa = open(f"{data_folder}/ALFFA_PUBLIC/ASR/SWAHILI/data/train/text", "r")
    f_test_alffa = open(f"{data_folder}/ALFFA_PUBLIC/ASR/SWAHILI/data/test/text", "r")
    ...
    for i in tqdm(range(len(text_alffa))):
        ...
        for j in range(len(wavs_alffa)):
            if wavs_alffa[j].split("/")[-1] == file_name+".wav":
                ...
                info = torchaudio.info(wav)
                duration = info.num_frames / info.sample_rate
                dic = {"wav" : wavs_alffa[j].replace(data_folder+"/", ""), "words" : str(words).replace("\n", ""), "duration" : duration}
...
def create_csv(
    orig_csv_file, csv_file, data_folder, accented_letters=False, language="darija"
):
    """
    ...
    for line in tzip(loaded_csv):
        ...
        ALFFA_LANGUAGES = ["amharic", "fongbe", "wolof"]
        if language in ALFFA_LANGUAGES or language == "swahili" or language == "multilingual":
