...
from common.dataio_new import dataio_prep, mulaw_quantize
...
class WavenetBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage, use_targets=True):
        ...
        if stage == sb.Stage.VALID:
            ...
            initial_value=mulaw_quantize(torch.tensor(0).float(),255)
            initial_input = torch.from_numpy(np.asarray(np.eye(256, 
            dtype='float32')[initial_value]))

            initial_input = initial_input.view(1, 1, 256)

            target=batch.target.data
            time_length = target.size(1)
            pred = self.hparams.model.incremental_forward(c=batch.mel.data.transpose(1,2),
            T=time_length, initial_input=initial_input.to(target.device),log_scale_min=-9.0)
        '''
        return pred
