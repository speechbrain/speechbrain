...
def prepare_dcase2019_task4(save_json_dir,
                      wav_dir,
                      meta_dir,
    ...
    for dataset in ['weak.tsv', 'synthetic.tsv', 'unlabel_in_domain.tsv']:
        ...
        missing_file = os.path.join(missing_file_dir, 'missing_files_'+dataset)
        if (dataset == 'unlabel_in_domain.tsv' and wav_dir_unlabel is not None):
            audio_folder = os.path.join(wav_dir_unlabel)# 'train', dataset.split('.')[0])
        else:
        ...
        if dataset=='weak.tsv':
            ...
            create_json(audio_folder, tsv_file, json_file, missing_file, toy=True)
    ...
    ## validation
    for dataset in ['validation.tsv', 'eval_dcase2018.tsv', 'test_dcase2018.tsv']:
        ...
        if "event_label" in head:
            ...
            for line in reader:
              if line[0] in missing_files['filename']:
                  ...
                  json_dict[f'{uttid}'] = {'filepath':os.path.join(wav_folder,line[0])}
                  json_dict[f'{uttid}'].update({'event_label':{'event_label_'f'{filename_dict[line[0]][0]}':line[1:]}})
                  json_dict[f'{uttid}'].update({'event_labels':[line[-1]]})
                  uttid += 1
              else:
                  filename_dict[line[0]][0] += 1
                  json_dict[f'{filename_dict[line[0]][1]}']['event_label'].update({'event_label_'f'{filename_dict[line[0]][0]}':line[1:]})
                  if not line[-1] in json_dict[f'{filename_dict[line[0]][1]}']['event_labels']:
                      json_dict[f'{filename_dict[line[0]][1]}']['event_labels'].append(line[-1])
            ...
            for line in reader:
                if line[0] in missing_files['filename']:
                    ...
                    for i in range(len(head)):
                        if i ==0:
                            json_dict[f'{uttid}'] = {'filepath': os.path.join(wav_folder,line[i])}
                        else:
                            if head[i]=='event_labels':
                                json_dict[f'{uttid}'].update({head[i]: line[i].split(',')})
                            else:
                              json_dict[f'{uttid}'].update({head[i]: line[i]})
                    json_dict[f'{uttid}'].update({'event_label': {'event_label_0':[0,0,line[-1].split(',')[0]]}})
                    uttid +=1
        else:
            uttid = 0
            for line in reader:
                if line[0] in missing_files['filename']:
                    ...
                    json_dict[f'{uttid}'].update({'event_label': {'event_label_0':[0,0, "unknown"]}})
                    uttid +=1

    
    if toy:
        toy_json_dict = {ks: json_dict[ks] for ks in list(json_dict.keys())[:10]}
