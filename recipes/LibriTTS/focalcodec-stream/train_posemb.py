...
"""Recipe for distilling full-context positional embeddings into causal ones.

To run this recipe:
> python train_posemb.py hparams/<path-to-config>.yaml
...
class Distillation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            posemb_input = self.hparams.activation_cache.inputs[
                "encoder.positional_embedding"
            ]
            posemb_output = self.hparams.activation_cache.outputs[
                "encoder.positional_embedding"
            ]
            feats = posemb_output[0]

        # Forward model
        hyp_feats, *state_ = self.modules.student_posemb(*posemb_input)
