...
"""Recipe for distilling a full-context encoder into a causal one.

To run this recipe:
> python train_encoder.py hparams/<path-to-config>.yaml
...
class Distillation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            self.hparams.encoder(sig, length=lens)
            feats = torch.stack(
                [
                    self.hparams.activation_cache.outputs[
                        f"encoder.layers.{i}"
                    ][0]
                    for i in range(len(self.hparams.distill_weights))
                ],
                dim=-1,
            )

        # Forward model
        self.modules.student_encoder(sig, length=lens)
        hyp_feats = torch.stack(
            [
                self.hparams.student_activation_cache.outputs[
                    f"encoder.layers.{i}"
                ][0]
                for i in range(len(self.hparams.distill_weights))
            ],
        ...
        # Ensure hyp_feats matches feats in length
        feat_len = feats.shape[1]  # Target length
        hyp_len = hyp_feats.shape[1]  # Current length

        if hyp_len > feat_len:  # Trim excess
            hyp_feats = hyp_feats[:, :feat_len]
        elif hyp_len < feat_len:  # Pad if too short
            pad_size = feat_len - hyp_len
            hyp_feats = torch.nn.functional.pad(
                hyp_feats, (0, 0, 0, pad_size)
            )  # Padding on time dim

        return hyp_feats, feats

    def compute_objectives(self, predictions, batch, stage):
        """Computes the objectives."""
        hyp_feats, feats = predictions

        _, lens = batch.sig

        # Reconstruction loss
        loss = self.hparams.rec_loss(
            hyp_feats, feats, length=lens, reduction="none"
        )
        loss = loss * torch.as_tensor(
            self.hparams.distill_weights, device=self.device
        )
        loss = loss.mean()
...
if __name__ == "__main__":
    ...
    # Freezing submodules
    hparams["student_encoder"].encoder.positional_embedding.requires_grad_(
        False
    )
    hparams["student_encoder"].encoder.relative_embedding.requires_grad_(False)
