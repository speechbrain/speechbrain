...
> python train.py hparams/train.yaml --kmeans_folder=/path/to/Kmeans/ckpt --data_folder=/path/to/LibriTTS
...
class HifiGanBrain(sb.Brain):
    ...
    def run_inference_sample(self, name, epoch):
        ...
        if self.hparams.use_tensorboard:
            ...
            self.tensorboard_logger.log_figure("{name}/mel_target", x)
            self.tensorboard_logger.log_figure("{name}/mel_pred", spec_out)
        else:
