...
class SEBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        if self.hparams.use_log1p:
            predict_wav = self.resynthesize((torch.expm1(predict_spec)))
        else:
            predict_wav = self.resynthesize(predict_spec)
    ...
    def compute_feats(self, wavs):
        ...
        # print('feat:{}'.format(feats.shape))
        # feats = sb.processing.features.spectral_magnitude(feats, power=0.5)
        # compute_cw  = sb.processing.features.ContextWindow(left_frames=self.hparams.contex, right_frames=self.hparams.contex)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # print(predictions["wav"].shape)
        # print(clean_wavs.shape)

        # print('predictions["wav"]:'.format(predictions["wav"].shape))
        # print('clean_wavs:'.format(clean_wavs.shape))
    ...
    def resynthesize(self, enhanced, normalize_wavs=True):
        ...
        # complex_predictions = torch.complex(enhanced[..., 0], enhanced[..., 1])

        # print(enhanced.shape)
        # enhanced = enhanced.unsqueeze(-1)
        # print(enhanced.dtype)
        # print(enhanced.shape)
        pred_wavs = self.hparams.compute_ISTFT(enhanced)
