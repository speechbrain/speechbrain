...
CMU_PHONES = {'N': 41, 'OW': 42, 'S': 43, 'IH': 3, 'G': 4, 'AH': 5, 'L': 6, 'EH': 7, 'M': 8, 'D': 9, 'B': 10, 'Y': 11, 'UW': 12, 'AE': 13, 'T': 14, 'AY': 15, 'K': 16, 'DH': 17, 'EY': 18, 'IY': 19, 'W': 20, 'R': 21, 'AA': 22, 'F': 23, 'Z': 24, 'AO': 25, 'NG': 26, 'V': 27, 'P': 28, 'HH': 29, 'UH': 30, 'TH': 31, 'AW': 32, 'CH': 33, 'JH': 34, 'ER': 35, 'OY': 36, 'SH': 37, 'ZH': 38, 'TS': 39, 'IX': 40, '<blank>': 0, '<bos>': 1, '<eos>': 2}
...
def prepare_syllabus(
    ...
    text_grids = glob(os.path.join(data_folder, "**/*.TextGrid"), recursive=True)
    ...
    for i, id in enumerate(text_grids_ids):
        if id not in audio_ids:
            msg = "{} does not have a corresponding audio file, remove it from the data preparation".format(id)
            logger.info(msg)
        else:
        ...
        if contains_all_fields:
            for field in use_fields:
                for syll in tg[field]:
                    ...
                    if len(label) > 0:
                        result = subprocess.run(["python", "lexconvert.py", "--phones2phones", "unicode-ipa", "cmu", str(label)], capture_output=True)
                        phones = result.stdout.decode('UTF-8').strip().replace("\n", "")
                        
                        phone_list = phones.split()
                        phone_list = [phn for phn in phone_list if phn in CMU_PHONES]
                        phones = " ".join(phone_list)
                        print(phones)

                        grid_name = grid.split(".TextGrid")
                        clip_name = grid_name[0].split("/")[-1] + ".{}-{}".format(syll.xmin, syll.xmax)
...
def process_audio_files(clips, audio_files, save_folder):
    ...
    for key, val in clips.items():
        ...
        audio_file_name = os.path.join(save_folder, text_grid.split(".TextGrid")[0].split("/")[-1] + ".wav")
        if not os.path.exists(audio_file_name):
            audio_file_name = os.path.join(save_folder, text_grid.split(".TextGrid")[0].split("/")[-1] + ".mp3")
