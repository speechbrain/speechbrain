...
class CRDNN(sb.nnet.containers.Sequential):
    ...
    def __init__(
        ...
        # self.append(torch.nn.Unflatten(1, (1,453)))

        if True:
            self.append(sb.nnet.containers.Sequential, layer_name="fc")
            self.fc.append(
                sb.nnet.linear.Linear,
                input_size=453,
                n_neurons=4,
            ...
            # )
            # self.fc.append(activation())
            self.append(torch.nn.Unflatten(1, (1,4)))
        # self.append(sb.nnet.containers.Sequential, layer_name="fc")
        # self.fc.append(
        #     DNN_Block,
        #     input_shape=(10,1,1,453),
        #     neurons=3,
        #     activation=activation,
        #     dropout=dropout,
        # )
...
class DNN_Block(sb.nnet.containers.Sequential):
    ...
    def __init__(
        self, input_shape, neurons, activation=torch.nn.LeakyReLU, dropout=0.15, combine_dims=False
    ):
        super().__init__(input_shape=input_shape)
        self.append(
...
if __name__ == "__main__":
    ...
    print(data.shape)
    model = CRDNN(input_size=F, cnn_channels=[32,64], dnn_neurons=128)
    output = model(data)
    print(output.shape)
    # # input_size = 257
    # # contex = 3
