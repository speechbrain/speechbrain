    ...
    def __init__(self,
        ...
        if self.rnn_type in ['LSTM', 'GRU']:
            self.rnn = getattr(nn, rnn_type)(self.input_size,
                                             ...
                                             batch_first=True, dropout=self.dropout, bidirectional=bidir)
        else:
            try:
...
class crn_att(torch.nn.Module):
    ...
    def __init__(self, input_size=161, output_size=3, rnn_type='LSTM', contex=0, bidir=False, rnn_size=128, projection=64, layers=2, dropout=0.25):
        ...
        self.rnn = RNN_Block(input_size=rnn_size, hidden_size=rnn_size, rnn_type=rnn_type, num_layers=1, bidir=bidir)
        self.decoder = Decoder()

        att_in = rnn_size*2 if bidir else rnn_size

        self.dropout = nn.Dropout(dropout)

        self.attention = Attention(input_size=att_in,output_size=4)
...
if __name__ == "__main__":
    ...
    print(data.shape)
    model = crn_att(input_size=F, rnn_type='GRU', bidir=True)
    output = model(data)
    print(output.shape)
    # input_size = 257
    # contex = 3
