...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage, retrain=False):
        ...
        if stage == sb.Stage.TRAIN and retrain is True:
            ...
            loss = self.hparams.compute_cost(outputs, command, lens)
            # print(loss)
            ...
            # Collect datagrad
            data_grad = feats.grad.data

            # Call FGSM Attack
            perturbed_data = fgsm_attack(feats, 1.0, data_grad)
            for n in range(len(perturbed_data)):
                if command[n] == 3:
                    perturbed_data[n, :] = feats[n, :]

            # Embeddings + classifier
            outputs = self.modules.embedding_model(perturbed_data)
            if "classifier" in self.modules.keys():
                outputs = self.modules.classifier(outputs)
            # print("outputs.size():{}".format(outputs.size()))
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs = self.compute_forward(batch, sb.Stage.TRAIN, retrain=True)
            loss = self.compute_objectives(outputs, batch, sb.Stage.TRAIN)
            loss.backward()
            if self.check_gradients(loss):
                self.optimizer.step()
