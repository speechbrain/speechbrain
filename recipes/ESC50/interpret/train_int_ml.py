...
class InterpreterESC50Brain(sb.core.Brain):
    ...
    def classifier_forward(self, X_stft_logpower):
        ...
        #class_pred = predictions.sort(1, descending=True)[1]
        class_pred = None
    ...
    def debug_files(self, X_stft, xhat, X_stft_logpower, batch, wavs, idn=0):
        ...
        plt.savefig(
            os.path.join(out_folder, f"reconstructions_{idn}.png"),
            format="png",
        )
        plt.close()
        ...
        if not self.hparams.use_melspectra:
            torchaudio.save(
                os.path.join(out_folder, f"interpretation_{idn}.wav"),
    ...
    def compute_objectives(self, pred, batch, stage):
        ...
        self.acc_metric.append(
            ids=torch.ones(predictions.shape[0]), predictions=predictions, target=targets
        )
    ...
    def on_stage_start(self, stage, epoch=None):
        ...
        @torch.no_grad()
        def accuracy_value(predictions, target):
            auprc = binary_auprc(predictions, target, num_tasks=target.shape[0]).mean(0, keepdim=True)
            return auprc
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            current_fid = 0 # self.inp_fid.summarize("average")
