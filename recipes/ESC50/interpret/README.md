...
Note that overlap type should be either `mixtures` (for contaminating signal to be set as other signals from ESC50), `ljspeech` (for contaminating signal to be set as speech), or `white_noise` (for contaminating signal to be set as white noise). Please refer to the L-MAC paper for the performance obtained in each setting. Note that for the `ljspeech` option the `ljspeech_path` is required, that is you need to specify ` --ljspeech_path /yourpath/ljspeech`, if you want to set the contaminating signals as human speech. 
