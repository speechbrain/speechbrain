...
class SEBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # noisy_feats, noisy_feats_contex = self.compute_feats(noisy_wavs)
        ...
        # mask = self.modules.model(noisy_feats_contex)
        # predict_spec = torch.mul(mask, noisy_feats)

        predict_wav = self.modules.model(noisy_wavs)[1]
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        clean_wavs = clean_wavs[:, :predictions["wav"].shape[1]]
        # clean_spec, _ = self.compute_feats(clean_wavs)
        ...
        loss = self.modules.model.loss(predictions["wav"], clean_wavs, loss_mode='SI-SNR')
