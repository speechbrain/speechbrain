...
def calculate_stft_frames(audio_duration, sr, hop_length):
    return int(sr*audio_duration // (sr*hop_length / 1000) + 1)
...
def create_lexicon_and_oov_csv(all_texts, data_folder, save_folder):
    ...
    # Create lexicon.csv
    header = "ID,duration,frames,char,phn\n"
    ...
    with open(lexicon_csv_path, "w") as f:
        ...
        for idx in range(len(lexicon_words)):
            ...
            frames = calculate_stft_frames(duration, SAMPLERATE, HOP_LENGTH)
            ...
            line = (
                ",".join([str(idx), str(duration), str(frames), graphemes, phonemes]) + "\n"
            )
            f.write(line)
...
def create_csv(
    ...
    csv_lines = [["ID", "duration", "frames", "wav", "spk_id", "wrd"]]
