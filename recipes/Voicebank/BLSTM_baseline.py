...
def read_pesq(clean_folder, enhanced_file, sr):
    ...
    clean_wav = wavfile.read(clean_file)[-1].astype(float)
    enhanced_wav = wavfile.read(enhanced_file)[-1].astype(float)

    pesq_score = pesq(sr, clean_wav, enhanced_wav, "wb")
    return pesq_score
...
class SEBrain(sb.core.Brain):
    def compute_forward(self, x, stage="train", init_params=False):
        ...
        """
        encoded = params.lstm(feats, init_params=init_params)
        encoded = params.linear1(encoded, init_params)
        encoded = params.activation(encoded)
        """
        out = params.model(feats, init_params=init_params)
        # out     = params.activation1(encoded)
    ...
    def generate_enhanced_waveform(self, data_set):
        with tqdm(data_set) as t:
            for i, batch in enumerate(t):
                ...
                enhanced_wave = enhanced_wave / torch.max(abs(enhanced_wave))

                torchaudio.save(
                    params.enhanced_folder + ids[0] + ".wav",
