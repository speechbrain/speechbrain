#!/usr/bin/env python3
"""Recipe for training a Transformer ASR system with Myst.
...
are used as basic recognition tokens. Training is performed on Myst dataset.
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if current_epoch % valid_search_interval == 0 or (
                ...
                predicted_words = [
                    tokenizer.decode_ids(utt_seq).lower().split(" ") for utt_seq in hyps
                ]
...
def dataio_prepare(hparams):
    ...
    valtest_datasets = [valid_data, test_data] #+ [i for k, i in test_datasets.items()]
    ...
    def text_pipeline(wrd):
        yield wrd
        tokens_list = tokenizer.encode_as_ids(wrd.upper())
        yield tokens_list
