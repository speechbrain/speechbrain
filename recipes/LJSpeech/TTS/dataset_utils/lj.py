...
class LJ:
    ...
    METADATA_COLS = ["file_name","label_normalized", "durations"]
    METADATA_FILE_NAME = "metadata.csv"
    DIR_WAV = "wavs"

    def __init__(self, file_path: str, dur_path: str):
        self.file_path = file_path
        self.wav_path = os.path.join(self.file_path, self.DIR_WAV)
        self.dur_path = dur_path

        with open(os.path.join(self.file_path, "metadata.csv"), 'r') as f:
            lines = f.read().split('\n')[:-1]
        char_set = set()
        for l in lines:
    ...
    def _convert_metadata_record(self, record):
        ...
        return {
            "ID": record["file_name"],
            "speaker_id": 0,
            "wav": os.path.join(self.wav_path, f"{record['file_name']}"),
    ...
    def _read_metadata(self):
        ...
        with open(self.metadata_file) as metadata_file:
            ...
            for row in reader:
                file_stem = Path(row[0]).stem
                duration_path = os.path.join(self.dur_path, file_stem+'.npy')
                row.append(duration_path)
...
def load(file_path, dur_path):
    ...
    return LJ(file_path, dur_path).to_dataset()
