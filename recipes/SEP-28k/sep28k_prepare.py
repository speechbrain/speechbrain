...
def download_sep28k(data_path):
    ...
    if not os.path.exists(os.path.join(data_path, "SEP28k-data.zip")):
        print(f"SEP-28k is missing. We are now downloading it. Be patient, the total size is 1.9GB. Takes 1,995,736 iterations.")
        print(f"**** NOW DOWNLOADING zip file *******")
        download_dropbox("https://www.dropbox.com/scl/fi/rpavffri0odb2g25bxy58/sep28k_clips.zip?rlkey=zfxpdrek642pxu0rj64qid7gh&st=xum8yxm1&dl=0", 
                         f"{temp_path}/SEP28k-data.zip")
    if not os.path.exists(os.path.join(data_path, "SEP-28k-Extended_clips.csv")):
        print(f"**** NOW DOWNLOADING csv file *******")
        download_dropbox("https://www.dropbox.com/scl/fi/amzp62bpj8zqo21kpmoqy/SEP-28k-Extended_clips.csv?rlkey=5ehd8wv1q2gyz32m2pyynlcn2&st=cb76g1r4&dl=0",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', dest='path', type=str, default="sep28k_data")
    args = parser.parse_args()
    prepare_sep28k(args.path)