...
def make_csv_for_each(
    ...
    """Prepare the csv fils for each subfolder of the Libri-Light splits
    ...
    """
    subpath_1, csv_file_folder, max_length = subpath_1_csv_file_folder_max_length
...
def processes_folder(
    ...
    """Prepare the csv fils for one of the Libri-Light splits
    ...
    max_length:
        The max length for each prepared audio clip
    n_processes:
...
def parse_args():
    ...
    parser.add_argument('--input_dir', type=str, default=None,
                        help="Path to the Libri-Light split after vad", required=True)
    parser.add_argument('--output_dir', type=str, default=None,
                        help="Path to the output folder of generated csv files", required=True)
    parser.add_argument('--max_length', type=float, default=20.2,
                        help="The max length for each prepared audio clip"
                             "(default is 20.2)")
...
if __name__ == "__main__":
    args = parse_args()
    processes_folder(args.input_dir, args.output_dir, args.max_length, args.n_processes)
