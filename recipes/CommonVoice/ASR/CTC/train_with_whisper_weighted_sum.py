...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        avg_feats=torch.stack(list(self.layers.values()),dim=-1)
        avg_feats=self.modules.weight_sum(avg_feats)
        avg_feats=torch.squeeze(avg_feats,dim=-1)
        weights= self.modules.weight_sum.linear1.w.weight
        print(weights)
        x = self.modules.enc(avg_feats)
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            ...
            self.weight_sum_optimizer.zero_grad()
            ...
            if should_step:
                ...
                self.scaler.unscale_(self.weight_sum_optimizer)
                ...
                if self.check_gradients(loss):
                    if self.optimizer_step > self.hparams.warmup_steps:
                        ...
                        self.scaler.step(self.weight_sum_optimizer)
            ...
            if should_step:
                if self.check_gradients(loss):
                    ...
                    if self.optimizer_step > self.hparams.warmup_steps:
                        ...
                        self.weight_sum_optimizer.step()
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            old_lr_weight_sum, new_lr_weight_sum = self.hparams.lr_annealing_weight_sum(
                stage_stats["loss"]
            )
            ...
            sb.nnet.schedulers.update_learning_rate(
                self.weight_sum_optimizer, new_lr_weight_sum
            )
            self.hparams.train_logger.log_stats(
                stats_meta={
                    ...
                    "lr_weight_sum": old_lr_weight_sum,
                },
    ...
    def init_optimizers(self):
        ...
        self.weight_sum_optimizer = self.hparams.weight_sum_opt_class(
            self.hparams.weight_sum.parameters()
        )
        ...
        if self.checkpointer is not None:
            ...
            self.checkpointer.add_recoverable("weight_sum_opt", self.weight_sum_optimizer)
...
if __name__ == "__main__":
    ...
    def get_layer(name):
        def hook(model, input, output):
            asr_brain.layers[name] = output[0].detach()
        return hook

    for i in range(hparams['whisper_encoder_layers']):
        asr_brain.modules.whisper.model.encoder.layers[i].register_forward_hook(get_layer(i))

    # asr_brain.modules.weight_sum.linear1.w.weight=torch.nn.init.zeros_(asr_brain.modules.weight_sum.linear1.w.weight)
