...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        for enc_rank in range(self.hparams.rank_locale+1) : 
            feats = hparams["linear_encs"][enc_rank](feats)
        x = hparams["rnn_decoders"][self.hparams.forced_decoder_locale](feats)
        x=x[0]
        # Compute outputs
        p_tokens = None
...
def test(hparams, run_opts, locales, wer_file="wer_test.txt"):
    ...
    for i, locale in enumerate(locales):
        ...
        hparams["rank_locale"] = i+ len(hparams["old_locales"]) 

        hparams["ctc_decoders"][locale].eval()
        hparams["rnn_decoders"][locale].eval()
        hparams["linear_encs"][i].eval()
...
def train(hparams, run_opts):
    ...
    hparams["rnn_decoders"]={}
    hparams["tokenizer_backup"] = {}
    hparams["linear_encs"]=[]
    training_locales = hparams["new_locales"]
    if hparams["train_with_old_locales"] : 
        training_locales = hparams["old_locales"] + training_locales #Start with the old locales first
    for i, locale in enumerate(training_locales):
        ...
        hparams["linear_encs"].append(VanillaNN(input_shape= [None, None, 1024], activation = hparams["activation"], dnn_neurons = hparams["dnn_neurons"], dnn_blocks = hparams["dnn_layers"]))
        hparams["rnn_decoders"][locale] =LSTM(   input_shape=  [None, None, 1024], num_layers = 2, bidirectional = True, dropout = 0.2, hidden_size = 1024)
    ...
    if hparams["train_with_old_locales"]: 
        for i, locale in enumerate(hparams["old_locales"]):
            ...
            hparams["rnn_decoders"][locale].to(asr_brain.device)
            hparams["linear_encs"][i].to(asr_brain.device)
            ...
            #No testing in the old locales phase

    for i, locale in enumerate(hparams["new_locales"]):
        # Add new decoding layers
        if hparams["train_with_old_locales"] : 
            rank = i + len(hparams["old_locales"])
        run_on_main(
            prepare_common_voice,
            kwargs={
                "locales": [locale],
        ...
        hparams["model"] =torch.nn.ModuleList([hparams["rnn_decoders"][locale], hparams["ctc_decoders"][locale], hparams["linear_encs"][rank]])
        # Set forced decoder locale

        hparams["forced_decoder_locale"] = locale
        hparams["rank_locale"] = rank
        # Here we create the datasets objects as well as tokenization and encoding
        label_encoder = hparams["tokenizer_backup"][locale]
        train_data, valid_data, test_data = dataio_prepare(hparams, label_encoder)
        ...
        hparams["rnn_decoders"][locale].to(asr_brain.device)
        hparams["linear_encs"][rank].to(asr_brain.device)
