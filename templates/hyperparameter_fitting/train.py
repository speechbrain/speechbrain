...
def dataio_prepare(hparams):
    ...
    for dataset in ["train", "valid", "test"]:
        ...
        sample = hparams.get(f"{dataset}_sample")
        if sample is not None:
            sample_ids = list(datasets[dataset].data_ids)
            sample_ids = sample_ids[:sample]
            datasets[dataset].data_ids = sample_ids
...
if __name__ == "__main__":
    ...
    with hp.hyperparameter_fitting(objective_key="CER") as hp_ctx:
