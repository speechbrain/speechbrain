...
class execute_computations(nn.Module):
    """
     ...
     Example:   # EX1: read audio file and add random noise
                ...
                exec_cfg={'class_name': 'speechbrain.py', 
                          'cfg_file': 'cfg/minimal_examples/features/compute_mfccs_example.cfg', 
                          'root_cfg': 'True'}
    ...
    def __init__(
        ...
        self.expected_options = {
            ...
            "add_connections": ("one_of(residual,skip,dense)", "optional", "None"),
            "connection_merge": ("one_of(sum,average,diff,concat,linear_comb)", "optional", "sum"),
        ...
        self.config_proc = read_config(self.cfg_file,
            ...
            for gpu in self.gpu_id:
                if gpu > torch.cuda.device_count() -1:
                    err_msg = (
                        ...
                        % (self.funct_name, gpu, torch.cuda.device_count(),torch.cuda.device_count()-1)
                    )
        ...
        # computation section 
        all_funct = list(self.config_proc['functions'].keys())+\
        ...
        stop_computations = self.detect_stop_computations(cmd_str,
                                                          self.stop_at)
        
        # Check at which computation the output variables can be returned
        last_out_var_computation = self.detect_stop_computations(cmd_str,
                                                                 self.out_var)
    ...
    def forward(self, inp):
        ...
        if self.save_folder is not None:
            ...
            for index, out in enumerate(self.out_var_lst):
                ...
                out_file=self.output_folder+'/'+self.funct_name+'/'+self.out_var[index]+'.pkl'
    ...
    def run_computations(self, inp):
        ...
        if len(self.out_var) > 0 and (self.n_loops > 1 or self.csv_file is not None):
    ...
    def detach_tensor_in_list(lst):
        """
         ---------------------------------------------------------------------
         core.execute_computations.detach_tensor_in_list (author: Mirco Ravanelli)
    ...
    def recover_iteration(self,):
         ...
         Example:   import torch
                    ...
                    print(computations.functions["training_nn"].recover_iteration())
         ...
         if self.recovery:
            ...
            self.recovery_path = self.global_config['output_folder'] + \
                                 '/recovery.pkl'
            ...
            if os.path.exists(self.recovery_path):
                ...
                if self.funct_name in self.recovery_dict:
                    ...
                    if 'loop_id' in self.recovery_dict[self.funct_name]:
                        ...
                        self.start_index = self.recovery_dict[
                                           self.funct_name]['loop_id'] + 1
    ...
    def store_iteration(self,index):
        ...
        if self.recovery:
            ...
            self.recovery_path = self.global_config['output_folder']+'/recovery.pkl'
        ...
        if hasattr(self.functions[funct_name], 'device'):
            ...
            if self.functions[funct_name].device is not None:
                ...
                if 'cuda' in self.functions[funct_name].device:
                    funct_device='cuda:'+str(self.functions[funct_name].gpu_id)
    ...
    def inp_to_device(self,funct_name,inp_lst):
        ...
        Example:   import torch 
                   ...
                   inp_dev=computations.inp_to_device('training_nn',[inp_tensor])
                   print(inp_dev)
    ...
    def set_eval_mode(self,funct_name):
        ...
        if inherit_parent_mode:
            ...
            if parent_name is not None:
                    if hasattr(self.functions[parent_name], 'funct_eval_mode'):
                        funct_eval_mode = self.functions[parent_name].funct_eval_mode
        ...
        if hasattr(self.functions[funct_name], 'multi_gpu_model_names'):
            if len(self.functions[funct_name].multi_gpu_model_names) > 0:
                for funct_gpu_name in self.functions[funct_name].multi_gpu_model_names:
                    self.functions[funct_gpu_name].funct_eval_mode = self.functions[funct_name].funct_eval_mode
    ...
    def manage_multi_gpu_init(self,funct_name,lib,argv):
        """
        ---------------------------------------------------------------------
        core.execute_computations.manage_multi_gpu_init (author: Mirco Ravanelli)
        ...
        if hasattr(self.functions[funct_name], 'multi_gpu'):
            ...
            if self.functions[funct_name].multi_gpu:
                    ...
                    for device_id in self.functions[funct_name].gpu_id[1:]:
                        ...
                        self.initialize_multi_gpu(funct_name, 'cuda:'+str(device_id), lib, argv[1:])
                        
                        # Updating multi_gpu_models flag with the current model
                        self.functions[funct_name].multi_gpu_models.append(self.functions[funct_name+'_cuda:'+str(device_id)])
                        self.functions[funct_name].multi_gpu_model_names.append(funct_name+'_cuda:'+str(device_id))
                ...
                if hasattr(self.functions[parent_name], 'multi_gpu'):
                         if self.functions[parent_name].multi_gpu:
                             if self.functions[funct_name].multi_gpu is None:
                                  for device_id in self.functions[parent_name].gpu_id[1:]:
    ...
    def initialize_multi_gpu(self, funct_name, device_id, lib, inp_lst):
        """
        ---------------------------------------------------------------------
        core.execute_computations.initialize_multi_gpu (author: Mirco Ravanelli)

        Description: this function initializes the function funct_name on 
                     another gpu device with the name funct_name_cuda:device_id.
    ...
    def multi_gpu_computation(self,funct_name, inp_lst):
        """
        ---------------------------------------------------------------------
        core.execute_computations.multi_gpu_computation (author: Mirco Ravanelli)
        ...
        for gpu in self.functions[funct_name].gpu_id[1:]:
            ...
            if hasattr(self.functions[funct_name], 'parameters'):
                if len(list(self.functions[funct_name].parameters()))>0:
                    # copy parameters:
                    self.functions[funct_name_gpu].load_state_dict(self.functions[funct_name].state_dict())
        ...
        inputs = nn.parallel.scatter(inp_lst, [0]* len(self.functions[funct_name].gpu_id))
        ...
        result=nn.parallel.gather(outputs, self.functions[funct_name].gpu_id[0])
                ...
                if 'global' not in line:
                    if 'functions' not in line:
                        if 'computations' not in line:
                            funct_name_lst.append(line.replace('[/','').replace(']','').replace('\t','').replace(' ','').replace('\n',''))
    ...
    def get_input_var(self):
        ...
        Description: this can be use to output the current input variables.
                     ...
                     cfg/minimal_examples/basic_processing/save_signals_with_noise.cfg
                     that is called from 
                     cfg/minimal_examples/basic_processing/minimal_processing_read_write_example_noise.cfg)
    ...
    def detect_stop_computations(self, cmd_str,stop_var):
        """
         ...
         Example:    from lib.core import execute_computations
                   ...
                   stop_index = computations.detect_stop_computations(cmd_str,['b'])
        ...
        if stop_var is not None and len(stop_var) > 0:
            ...
            for node in ast.walk(cmd_parsed):
                if isinstance(node, ast.Name) and \
                   isinstance(node.ctx, ast.Store) and node.id in stop_var:
    ...
    def add_dense(self,cmd_append,block_id,out_lst,lin_comb_cnt):
        ...
        if self.connection_merge == 'linear_comb':
            ...
            cmd_append='\n'+cmd_append+self.out_var[0]+'_'+str(block_id)+'_dense=self.run_function("linear_comb_'+ str(lin_comb_cnt)+'",'+  cmd_dense[:-1]+')\n'
            lin_comb_cnt=lin_comb_cnt+1
