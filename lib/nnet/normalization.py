...
class normalize(nn.Module):
    ...
    def __init__(
        ...
        self.expected_options = {
            ...
            "norm_type": ("one_of(batchnorm,layernorm,groupnorm,instancenorm,localresponsenorm)", "mandatory"),
            "recovery": ("bool", "optional","True"),
            "initialize_with": ("str", "optional","None"),
        ...
        if first_input is not None:
            ...
            if self.norm_type == 'groupnorm':
                ...
                self.norm=torch.nn.GroupNorm(self.num_groups, 
                                             n_ch, 
                                             eps=self.eps, 
                                             affine=self.affine)
            ...
            if self.norm_type == 'layernorm':
                self.norm = torch.nn.LayerNorm(first_input[0].size()[1:-1], 
                                               eps=self.eps,
            ...
            if self.norm_type == 'localresponsenorm':
                self.norm = torch.nn.LocalResponseNorm(self.neigh_ch,
                                                       alpha=self.alpha, 
                                                       beta=self.beta, 
                                                       k=self.k)
    ...
    def batchnorm(self,first_input):
        ...
        if len(first_input[0].shape) <= 3 :
            ...
            norm = nn.BatchNorm1d(fea_dim,eps=self.eps, 
        ...
        if len(first_input[0].shape) == 4 :
            ...
            norm = nn.BatchNorm2d(fea_dim,eps=self.eps, 
                                      momentum=self.momentum, 
                                      affine=self.affine, 
                                      track_running_stats=self.track_running_stats)
        ...
        if len(first_input[0].shape) == 5 :
            ...
            norm = nn.BatchNorm3d(fea_dim,eps=self.eps, 
                                      momentum=self.momentum, 
                                      affine=self.affine, 
                                      track_running_stats=self.track_running_stats)  
    ...
    def instancenorm(self,first_input):
        ...
        if len(first_input[0].shape) == 3 :
            # 1D case
            norm = nn.InstanceNorm1d(fea_dim,eps=self.eps, 
                                      momentum=self.momentum, 
                                      track_running_stats=self.track_running_stats)
        ...
        if len(first_input[0].shape) == 4 :
            # 2D case
            norm = nn.InstanceNorm2d(fea_dim,eps=self.eps, 
                                      momentum=self.momentum, 
                                      affine=self.affine, 
                                      track_running_stats=self.track_running_stats)
        ...
        if len(first_input[0].shape) == 5 :
            # 3D case
            norm = nn.InstanceNorm3d(fea_dim,eps=self.eps, 
                                      momentum=self.momentum, 
                                      track_running_stats=self.track_running_stats)  
...
class normalize_posteriors(nn.Module):
    """
     ...
     Example:   import torch
                ...
                # normalization initialization
                config={'class_name':'lib.nnet.normalization.normalize_posteriors',
                        'count_lab':'phn'}
    ...
    def __init__(
        ...
        if self.count_lab in global_config['label_dict']:
            self.count_dict=global_config['label_dict'][self.count_lab]['counts']
