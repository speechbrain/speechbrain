...
class compute_cost(nn.Module):
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - cost_type (one_of(nll,mse,l1,ctc,error), mandatory):
    ...
    def __init__(
        ...
        self.expected_inputs = ['torch.Tensor','torch.Tensor',['torch.Tensor','list']]
        ...
        for i, cost in enumerate(self.costs):
            ...
            if self.avoid_pad[i]:
                ...
                for i in range(N_snt):
                    ...
                    actual_size = int(torch.round(lengths[i] * lab_curr.shape[0]))
            ...
            # (e.g, when we sort the sentences before creating the batches)
                ...
                if self.cost_type[i] == 'ctc':
                    ...
                    input_lengths = torch.round(lengths[0]*prob_curr.shape[0]).int()
                    lab_lengths = torch.round(lengths[1]*lab.shape[-1]).int()
                    
                    # ctc cost computation
                    ctc_cost = cost(prob_curr, lab_curr, input_lengths, lab_lengths)
                    ...
                    if reshape:
                        lab_curr=lab.reshape(lab.shape[0]*lab.shape[1]).long()
                        prob_curr=prob.transpose(1,2)
                        prob_curr=prob_curr.reshape(prob_curr.shape[0]*prob_curr.shape[1],prob_curr.shape[2])
