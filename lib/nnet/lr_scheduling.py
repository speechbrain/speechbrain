...
class lr_annealing(nn.Module):
    """
     ...
     Input (init):  - config (type, dict, mandatory):
                       it is a dictionary containing the keys described below.
                           ...
                           - improvement_threshold (float, optional, Default: 0.0025):
    ...
    def __init__(
        ...
        self.expected_options = {
            ...
            "annealing_type": ("one_of(constant,time_decay,step_decay,exp_decay,newbob,custom)", 
                               "mandatory"),
    ...
    def forward(self, input_lst):
        ...
        for opt in self.optimizers:
            ...
            if self.annealing_type=='time_decay':
                next_lr=self.lr_initial/(1+self.epoch_decay*(current_epoch))
            
            # Managing step_decay
            if self.annealing_type=='step_decay':
                next_lr=self.lr_initial*math.pow(self.epoch_decay, math.floor((1+current_epoch)/self.epoch_drop))
            
            # Managing exp_decay
            if self.annealing_type=='exp_decay':
                next_lr=self.lr_initial*math.exp(-self.exp_decay*current_epoch)
    ...
    def get_optimizer_list(self):
        """
         ----------------------------------------------------------------------
         nnet.lr_scheduling.lr_annealing.get_optimizer_list (author: Mirco Ravanelli)
    ...
    def check_annealing_type(self):
        """
         ----------------------------------------------------------------------
         nnet.lr_scheduling.lr_annealing.check_annealing_type (author: Mirco Ravanelli)
